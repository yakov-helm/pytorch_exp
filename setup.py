...
def configure_extension_build():
    ...
    if cmake_cache_vars["BUILD_CAFFE2"]:
        ...
        if cmake_cache_vars["USE_CUDA"]:
            extensions.append(
                Extension(
                    name=str("caffe2.python.caffe2_pybind11_state_gpu"), sources=[]
                ),
            )
        if cmake_cache_vars["USE_ROCM"]:
            extensions.append(
                Extension(
                    name=str("caffe2.python.caffe2_pybind11_state_hip"), sources=[]
                ),
            )
    if cmake_cache_vars["BUILD_FUNCTORCH"]:
        extensions.append(
            Extension(name=str("functorch._C"), sources=[]),
        )
    if cmake_cache_vars["BUILD_NVFUSER"]:
        extensions.append(
            Extension(name=str("nvfuser._C"), sources=[]),
        )
