...
""",
    ...
    def test_grad_scaling_unscale_sparse(self, device="cuda", dtype=torch.float):
        ...
        i = torch.tensor([[0, 1, 1], [2, 0, 2]], device="cuda", dtype=torch.int64)
        v = torch.tensor([16.0, 32.0, 64.0], device="cuda", dtype=torch.float)
        ...
        p.grad = torch.sparse_coo_tensor(
            i, v, torch.Size([2, 3]), device="cuda", dtype=torch.float16
        )
        found_inf.zero_()
        found_inf = scaler._unscale_grads_(opt, inv_scale, found_inf, True)[cur]
        self.assertEqual(found_inf, 1.0)
    ...
    def _create_scaling_models_optimizers(
        self, device="cuda", optimizer_ctor=torch.optim.SGD, optimizer_kwargs=None
    ):
    ...
    def _grad_scaling_autocast_test(
        self, *, atol=1e-3, optimizer_ctor=torch.optim.SGD, optimizer_kwargs=None
    ):
        try_pickle = False
    ...
    def test_grad_scaling_autocast_foreach(self):
        for optimizer_ctor in (torch.optim.SGD, torch.optim.Adam, torch.optim.AdamW):
            self._grad_scaling_autocast_test(
                optimizer_ctor=optimizer_ctor, optimizer_kwargs={"foreach": True}
            )

    def test_grad_scaling_autocast_fused(self):
        for optimizer_ctor in (torch.optim.Adam, torch.optim.AdamW):
            self._grad_scaling_autocast_test(
                optimizer_ctor=optimizer_ctor, optimizer_kwargs={"fused": True}
            )
        ...
        class Optimizer1(_PlaceHolderOptimizer):
            def step(self, closure=None, *, grad_scaler=None):
                self.tester.assertTrue(
                    isinstance(grad_scaler, torch.cuda.amp.GradScaler)
                )
    ...
    def test_autocast_torch_bf16(self):
        with torch.backends.cudnn.flags(enabled=True, deterministic=True):
            for op_with_args in self.autocast_lists.torch_fp16:
                ...
                should_error_from_not_implemented = (
                    should_error_from_cudnn
                    or "thnn" in op
                    or "fused" in op
                    or "gru" in op
                    or op == "_thnn_fused_lstm_cell"
                    or op == "lstm_cell"
                )
                if not skip_test:
    ...
    @unittest.skipIf(
        (not TEST_GRAPH), "CUDA >= 11.0 or ROCM >= 5.3 required for graphs"
    )
...
"""
    ...
    @unittest.skipIf(
        (not TEST_GRAPH) or TEST_CUDAMALLOCASYNC,
            ...
            for i in range(2):
                for stat, expected in zip(stats_to_check, expecteds):
                    ...
                    self.assertEqual(
                        ...
                        + stat
                        + " = {}, expected = {}, numel = {}".format(
                            current, expected, numel
                        ),
                    )
    ...
    def test_graph_adam_adamw_with_explicitly_capturable_param_groups(self):
        ...
        for optimizer, second_param_group_capturable in product(
            (torch.optim.Adam, torch.optim.AdamW), (True, False)
        ):
            ref_p1, param1 = [
                    ...
                    if "history" in b:
                        for h in b["history"]:
                            if h["real_size"] == 311 * 411 * 4:
                                self.assertTrue(
                                    "test_cuda" in h["frames"][0]["filename"]
                                )
                                found_it = True
