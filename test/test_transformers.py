...
isSM86Device = torch.cuda.is_available() and torch.cuda.get_device_capability() == (
    8,
    6,
)
...
class TestTransformers(NNTestCase):
    ...
    )  # TODO: make this work for both cuda and cpu
        ...
        with maybe_autocast:
            for input, src_key_padding_mask in input_mask_pairs:
                ...
                torch.testing.assert_close(
                    fastpath_output_expanded, slowpath_output, rtol=1e-7, atol=1e-5
                )
...
class TestSDPA(NNTestCase):
    ...
    def convert_flash_attn_S_to_softmax(
        self, S, query_padding_mask, key_padding_mask, head_dim, causal=False
    ):
        ...
        mmas_n = (
            ...
            else (block_size // warps_n // 16)
        )

        S_converted = S_flat.view(
            S_flat.shape[0],
            S_flat.shape[1],
            loop_steps,
            seqlen_q // 16,
        ...
        S_converted = S_converted.reshape(
            S_flat.shape[0],
            S_flat.shape[1],
            (seqlen_q // 16 * 2 * 8),
            (loop_steps * mmas_n * warps_n * 2 * 4 * 2),
        )
        ...
        q_mask_fill = ~query_padding_mask.view(
            query_padding_mask.shape[0], 1, query_padding_mask.shape[1], 1
        )
        ...
        k_mask_fill = ~key_padding_mask.view(
    ...
    def test_scaled_dot_product_attention_fused_kernels(
        self, type: str, is_contiguous: bool, head_dims_match: bool
    ):
        rand_tensor = partial(
            self.rand_tensor, type=type, device="cuda", dtype=torch.float16
        )
        ...
        self.assertEqual(
            actual[0].contiguous(), math_ref[0].contiguous(), atol=1e-3, rtol=1e-2
        )
    ...
    def test_scaled_dot_product_attention_fused_kernels_packed(
        self, type: str, is_contiguous: bool
    ):
        rand_tensor = partial(
            self.rand_tensor, type=type, device="cuda", dtype=torch.float16, packed=True
        )
    ...
    @parametrize("type", ["dense", "nested"])
    @parametrize(
        "fused_kernel", [SDPBackend.FLASH_ATTENTION, SDPBackend.EFFICIENT_ATTENTION]
    )
    def test_scaled_dot_product_attention_fused_kernels_packed_accuracy(
        self, type: str, fused_kernel: str
    ):
        ...
        def rand_nt(shape):
            ...
            return (
                torch.nested.nested_tensor(tensors, device="cuda", dtype=torch.float32),
    ...
    def test_sdp_mem_efficient_grad_against_math(
        self, contiguous_inputs: bool, is_causal: bool
    ):
        batch_size, seq_len, num_heads, head_dim = 4, 4, 2, 16
        rand_tensor = partial(
            self.rand_tensor,
        ...
        with sdp_kernel(
            enable_math=False, enable_mem_efficient=True, enable_flash=False
        ):
    ...
    def test_sdp_flash_attention_grad_against_math(
        self, contiguous_inputs: bool, is_causal: bool, dtype: torch.dtype
    ):
        batch_size, seq_len, num_heads, head_dim = 4, 4, 2, 16
        rand_tensor = partial(
            self.rand_tensor,
    ...
    def test_fused_sdp_choice(self, type: str):
        ...
        for dtype in floating_types_and_half():
            make_tensor = partial(
                self.rand_tensor, type=type, device=device, dtype=dtype
            )
            size = (2, 2, 3, 4)
        ...
        if PLATFORM_SUPPORTS_FUSED_SDPA:
            ...
            device = "cuda"
            make_tensor = partial(
                self.rand_tensor, device=device, dtype=torch.float16, packed=True
            )
            ...
            make_tensor = partial(
                self.rand_tensor, device=device, dtype=torch.float32, packed=True
            )
    ...
    @unittest.skipIf(
        not PLATFORM_SUPPORTS_FUSED_SDPA, "Platform does not support fused SDPA"
    )
    ...
    @unittest.skipIf(
        not PLATFORM_SUPPORTS_FUSED_SDPA or not isSM86Device,
    ...
    def test_memory_efficeint_sm86_failure(self):
        ...
        dtype = torch.float16
        make_tensor = partial(
            self.rand_tensor, type="dense", device=device, dtype=dtype
        )
    ...
    def test_flash_backward_sm86_headdim128(self):
        ...
        q, k, v = make_tensor(size), make_tensor(size), make_tensor(size)
        with sdp_kernel(
            enable_mem_efficient=False, enable_flash=True, enable_math=False
        ):
    ...
    @unittest.skipIf(
        not PLATFORM_SUPPORTS_FUSED_SDPA,
        "Platform does not support fused scaled dot product attention",
    )
    ...
    def test_invalid_fused_inputs_invalid_dtype(self, kernel: SDPBackend):
        with sdp_kernel(**self.backend_map[kernel]):
            ...
            size = (2, 2, 3, 16)
            make_tensor = partial(
                self.rand_tensor, type="dense", device=device, dtype=torch.float64
            )
            q, k, v = make_tensor(size), make_tensor(size), make_tensor(size)
            self.assertRaises(
                RuntimeError,
    ...
    def test_invalid_fused_inputs_attn_mask_present(self, kernel: SDPBackend):
        with sdp_kernel(**self.backend_map[kernel]):
            ...
            size = (2, 2, 3, 16)
            make_tensor = partial(
                self.rand_tensor, type="dense", device=device, dtype=torch.float16
            )
            q, k, v = make_tensor(size), make_tensor(size), make_tensor(size)
    ...
    def test_unaligned_tensors(self):
        ...
        shape = (2, 2, 8, 5)
        make_tensor = partial(
            self.rand_tensor, shape=shape, type=type, device=device, dtype=dtype
        )
    ...
    def test_incompatible_mask(self):
        ...
        def func():
            return mha(
                qkv, qkv, qkv, need_weights=False, key_padding_mask=kpm, attn_mask=am
            )
    ...
    def test_mem_efficient_attention_vs_math_ref_grads(
        ...
        query_ref = (
            query.clone().detach().to(higher_precision_dtype).requires_grad_(True)
        )
        key_ref = key.clone().detach().to(higher_precision_dtype).requires_grad_(True)
        value_ref = (
            value.clone().detach().to(higher_precision_dtype).requires_grad_(True)
        )
        ...
        output_ref_atol = max(
            torch.abs(out_deviation).max().item(), default_atol[out.dtype]
        )
        output_ref_rtol = max(get_rtol(out_ref, out_lp_ref), default_rtol[out.dtype])

        grad_q_deviation = query_ref.grad - query_ref_lp.grad
        grad_q_ref_atol = max(
            torch.abs(grad_q_deviation).max().item(), default_atol[out.dtype]
        )
        grad_q_ref_rtol = max(
            get_rtol(query_ref.grad, query_ref_lp.grad), default_rtol[out.dtype]
        )

        # TODO: Investigate why grad_k needs larger tolerances
        grad_k_deviation = key_ref.grad - key_ref_lp.grad
        grad_k_ref_atol = max(
            7 * torch.abs(grad_k_deviation).max().item(), 7 * default_atol[out.dtype]
        )
        grad_k_ref_rtol = max(
            7 * get_rtol(key_ref.grad, key_ref_lp.grad), 7 * default_rtol[out.dtype]
        )

        grad_v_deviation = value_ref.grad - value_ref_lp.grad
        grad_v_ref_atol = max(
            torch.abs(grad_v_deviation).max().item(), default_atol[out.dtype]
        )
        grad_v_ref_rtol = max(
            get_rtol(value_ref.grad, value_ref_lp.grad), default_rtol[out.dtype]
        )
    ...
    def test_flash_attention_vs_math_ref_grads(
        ...
        query_padding_mask = torch.ones(1, seq_len_q, device="cuda", dtype=torch.bool)
        key_padding_mask = torch.ones(1, seq_len_k, device="cuda", dtype=torch.bool)
        ...
        grad_q_deviation = query_ref.grad - query_ref_lp.grad
        grad_q_ref_atol = max(
            4 * torch.abs(grad_q_deviation).max().item(), default_atol[out.dtype]
        )
        grad_q_ref_rtol = max(
            get_rtol(query_ref.grad, query_ref_lp.grad), default_rtol[out.dtype]
        )
        ...
        grad_k_deviation = key_ref.grad - key_ref_lp.grad
        grad_k_ref_atol = max(
            torch.abs(grad_k_deviation).max().item(), default_atol[out.dtype]
        )
        grad_k_ref_rtol = max(
            get_rtol(key_ref.grad, key_ref_lp.grad), default_rtol[out.dtype]
        )
    ...
    def test_fused_kernels_seq_len_1_inputs(self, fused_kernel):
        ...
        rand_nested_tensor = partial(
            self.rand_tensor, type="nested", device="cuda", dtype=torch.float16
        )
    ...
    def test_fused_kernels_nested_broadcasting(
        ...
        rand_nested_tensor = partial(
            self.rand_tensor, type="nested", device="cuda", dtype=dtype
        )
    ...
    def test_fused_kernels_nested_broadcasting_query_dense(self):
        rand_nested_tensor = partial(
            self.rand_tensor, type="nested", device="cuda", dtype=torch.float32
        )
