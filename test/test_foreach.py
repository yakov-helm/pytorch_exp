...
class TestForeach(TestCase):
        ...
        for i, sample in enumerate(
            ...
            if op.supports_autograd and dtype in floating_types() and not zero_size:
                ...
                try:
                    ...
                    if isinstance(rhs_arg, list) and isinstance(
                        rhs_arg[0], torch.Tensor
                    ):
                        self.assertEqual(
                            [t.grad for t in rhs_arg], [t.grad for t in ref_rhs_arg]
                        )
            if (
            ...
            if op.supports_autograd and dtype in floating_types() and not zero_size:
                ...
                try:
                    ...
                    for op_list, ref_list in zip(rhs_arg, ref_rhs_arg):
                        if isinstance(op_list, list) and isinstance(
                            op_list[0], torch.Tensor
                        ):
                            self.assertEqual(
                                [t.grad for t in op_list], [t.grad for t in ref_list]
                            )
    ...
    def test_unary_op(self, device, dtype, op, is_fastpath):
        ...
        for sample in samples:
            ...
            inputs = [sample.input]
            disable_fastpath = (
                op.name == "_foreach_abs" and dtype in complex_types()
            ) or sample.kwargs.pop("disable_fastpath")
            self.assertEqual(
                ref(inputs),
                wrapped_op(
                    inputs,
    ...
    def test_reduce_op(self, device, dtype, op, is_fastpath):
        for sample in op.sample_inputs(device, dtype, noncontiguous=not is_fastpath):
            ...
            if op.supports_autograd and dtype in floating_types() and not zero_size:
                ...
                sum(
                    wrapped_op((tensors,), False, False, ord=ord, zero_size=zero_size)
                ).backward()
    ...
    def test_binary_op_tensors_on_different_devices(self, device, dtype, op):
        ...
        # `tensors2`: ['cuda', 'cpu']
        _cuda_tensors = list(
            op.sample_inputs(device, dtype, num_input_tensors=[2], same_size=True)
        )[0].input
        _cpu_tensors = list(
            op.sample_inputs("cpu", dtype, num_input_tensors=[2], same_size=True)
        )[0].input
    ...
    def test_pointwise_op_tensors_on_different_devices(self, device, dtype, op):
        ...
        _cpu_tensors = list(
            op.sample_inputs("cpu", dtype, num_input_tensors=[3], same_size=True)
        )[0].input
    ...
    def test_foreach_l2_large_value_input(self, device, dtype, op):
        ...
        inputs = (
            [
                t * scaler
                for t in list(op.sample_inputs(device, dtype, [N], low=1))[0].input
            ],
        )
    ...
    def test_lerp(self, device, dtype, op, is_fastpath):
        for sample in op.sample_inputs(device, dtype, noncontiguous=not is_fastpath):
            ...
            if (
                dtype in integral_types()
                or dtype == torch.bool
                or (not self.is_cuda and dtype == torch.half)
            ):
                with self.assertRaises(RuntimeError):
            ...
            inplace_actual = inplace_op(
                inplace_inputs, self.is_cuda, is_fastpath, **kwargs
            )
            ...
            if op.supports_autograd and dtype in floating_types() and not zero_size:
                ...
                sum(
                    wrapped_op(
                        (transformed_sample.input, *inputs[1:]),
                        False,
                        False,
                        **kwargs,
                        zero_size=zero_size,
