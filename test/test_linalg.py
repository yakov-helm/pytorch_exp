...
class TestLinalg(TestCase):
    ...
    def test_norm_complex(self, device, dtype):
        def gen_error_message(input_size, ord, keepdim, dim=None):
            return (
                "complex norm failed for input size %s, ord=%s, keepdim=%s, dim=%s"
    ...
    def test_linalg_solve_triangular(self, device, dtype):
        if TEST_WITH_ROCM and dtype is torch.float32:
            raise unittest.SkipTest(
                "Skipping for ROCm for Magma backend; unskip when hipSolver backend is enabled"
            )
    ...
    @skipCUDAVersionIn(
        [(11, 3), (11, 6), (11, 7)]
    )  # Tracked in https://github.com/pytorch/pytorch/issues/70111
    @dtypes(*floating_and_complex_types())
    @precisionOverride(
        {
            torch.float32: 1e-2,
    ...
    def test_ormqr_errors_and_warnings(self, device, dtype):
        test_cases = [
            ...
            (
                ...
                (20,),
                (10, 6),
                r"other.shape\[-2\] must be greater than or equal to tau.shape\[-1\]",
            ),
            (
                (6, 6),
                (5,),
        ...
        def generate_reflectors_and_tau(A):
            ...
            if A.numel() > 0:
                ...
                for A_i, reflectors_i, tau_i in zip(
                    ...
                    reflectors_tmp, tau_i[:] = map(
                        torch.from_numpy, np.linalg.qr(A_i, mode="raw")
                    )
        ...
        for ms, batch, pivot, singular, fn in itertools.product(
            ...
            if (
                ...
                and (
                    ...
                    or torch.version.cuda.split(".") >= ["11", "3"]
                )
            ):
...
""".format(
    ...
    def test__int_mm(self, device, k, n, use_transpose_a, use_transpose_b):
        ...
        SM86OrLater = (
            torch.cuda.is_available() and torch.cuda.get_device_capability() >= (8, 6)
        )
        if version == (11, 7):
    ...
    @skipCUDAIf(
        torch.version.cuda is not None and torch.version.cuda.split(".") < ["11", "3"],
        "There's a bug in cuSOLVER < 11.3",
    )
    ...
    def test_det_logdet_slogdet(self, device, dtype):
        ...
        def test_single_det(M, target, desc):
            ...
            self.assertEqual(
                ...
                atol=1e-6,
                rtol=0,
                msg="{} (linalg_slogdet)".format(desc),
            )
            ...
            if sdet.item() < 0:
                ...
                self.assertEqual(
                    ...
                    atol=1e-6,
                    rtol=0,
                    msg="{} (logdet non-negative case)".format(desc),
                )
