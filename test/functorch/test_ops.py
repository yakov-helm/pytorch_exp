...
class TestOperators(TestCase):
    ...
    @opsToleranceOverride(
        ...
        (
            ...
            tol1(
                "svd_lowrank",
                {torch.float32: tol(atol=3e-05, rtol=3e-04)},
                device_type="cuda",
            ),
            tol1(
                "linalg.tensorsolve",
    ...
    def jvp_opinfo_test(
        self, fn, sample, output_process_fn, clone_inputs, fixme_ref_jvp_local
    ):
        ...
        noncontig_fn, primals = normalize_op_input_output2(
            fn, noncontig_args, noncontig_kwargs, output_process_fn, requires_grad=True
        )
        ...
        vmapjvpall_fail.union(
            {
                ...
                ),  # hit vmap fallback, which is disabled
            }
        ),
    )
    @toleranceOverride({torch.float32: tol(atol=1e-04, rtol=1e-04)})
        ...
        vjp_fail.union(
            {
                ...
                xfail(
                    "nn.functional.logsigmoid", ""
                ),  # not differentiable w.r.t. buffer
                ...
                ),  # NYI: log_sigmoid_backward
                xfail(
                    "nn.functional.soft_margin_loss", ""
                ),  # NYI: forward-AD for log_sigmoid_backward
        ...
        vjp_fail.union(
            {
                ...
                xfail("native_dropout_backward"),
                decorate(
                    "linalg.svd", decorator=skipIfRocm
                ),  # https://github.com/pytorch/pytorch/issues/97256
                decorate(
                    "svd", decorator=skipIfRocm
                ),  # Flaky tensor-likes are not close error on ROCm, adjust tolerance?
            }
        ),
    )
    @ops(op_db + additional_op_db + autograd_function_db, allowed_dtypes=(torch.float,))
