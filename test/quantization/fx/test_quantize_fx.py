...
class TestFuseFx(QuantizationTestCase):
    ...
    def test_fusion_pattern_with_multiple_inputs(self):
        ...
        for node in m.graph.nodes:
            if (
                node.op == "call_module"
                and type(named_modules[node.target]) == torch.nn.Conv2d
            ):
                self.assertTrue(
                    len(node.args) == 2
                ), "Expecting the fused op to have two arguments"
...
class TestQuantizeFx(QuantizationTestCase):
    ...
    def test_qconfig_mapping_set_module_name_object_type_order(self):
        ...
        self.assertEqual(
            list(qconfig_mapping.module_name_object_type_order_qconfigs)[0], key1
        )
        self.assertEqual(
...
class TestQuantizeFxModels(QuantizationTestCase):
    ...
    def _test_model_impl(
        ...
        if eager_quantizable_model is not None:
            ...
            if mode == "ddp":
                mp.spawn(
                    run_ddp, args=(world_size, qeager), nprocs=world_size, join=True
                )
            elif mode == "qat":
