...
class TestQuantizedOps(TestCase):
    ...
    def test_qelu(self, X, alpha):
        ...
        self.assertEqual(qY, qY_hat, msg="F.elu failed ({} vs {})".format(qY, qY_hat))
    ...
    def test_qcelu(self, X, alpha):
        ...
        self.assertEqual(qY, qY_hat, msg="F.celu failed ({} vs {})".format(qY, qY_hat))
            ...
            for test_case in test_cases:
                ...
                enough_unique_vals_in_each_layer = (
                    sum(
                        ...
                        if (
                            dqX[i].shape[0] < 5
                            or float(torch.unique(dqX[i]).shape[0]) / dqX[i].shape[0]
                            > 0.01
                        )
                        else 0
    ...
    def test_qtanh(self, X):
        ...
        self.assertEqual(qY, qY_hat, msg="TanH failed: {} vs. {}".format(qY, qY_hat))
    ...
    def test_qclamp(self, X, min_val, max_val):
        ...
        for name, op in ops_under_test.items():
            ...
            self.assertEqual(
                qY_clamp, qY_clamp_hat, msg="{} qclamp failed".format(name)
            )
    ...
    def test_hardtanh(self, X, min_val, max_val):
        with override_quantized_engine("fbgemm"):
            ...
            ops_under_test = {
                "nn.quantized.functional.hardtanh": torch.ao.nn.quantized.functional.hardtanh,
            ...
            ops_under_test_inplace = {
                "inplace nn.quantized.functional.hardtanh": torch.ao.nn.quantized.functional.hardtanh,
        ...
        for test_case in test_cases:
            ...
            for memory_format in torch.channels_last, torch.contiguous_format:
                ...
                self.assertEqual(
                    qY,
                    qY_hat,
                    msg="Hardswish failed: {} vs {}, {}".format(
                        qY, qY_hat, torch.backends.quantized.engine
                    ),
                )
    ...
    def _test_binary_op_scalar_relu(
        ...
        self.assertEqual(
            ...
            "{} vs {}".format(
                binary_op_name, C_relu_ref.dequantize(), C_relu_hat.dequantize()
            ),
        )
    ...
    @unittest.skip(
        "Local only - currently the test_qadd_relu_cudnn op is bulid "
    ...
    @unittest.skipIf(not TEST_CUDNN, "cudnn is not enabled.")
    @unittest.skip(
        "Local only - currently the test_qadd_relu_cudnn_nhwc op is bulid "
        "with USE_EXPERIMENTAL_CUDNN_V8_API, we can enable the test "
        "after it is built by default"
    )
    ...
    def test_max_pool1d(self, X, kernel, stride, dilation, padding, ceil_mode):
        ...
        for name, op in ops_under_test.items():
            ...
            self.assertEqual(
                a_ref, a_hat.dequantize(), msg="{} results are off".format(name)
            )
    ...
    @unittest.skipIf(not TEST_CUDNN, "cudnn is not enabled.")
    @unittest.skip(
        "Local only - currently the qconv2d_cudnn op is bulid "
        "with USE_EXPERIMENTAL_CUDNN_V8_API, we can enable the test "
        "after it is built by default"
    )
    ...
    def test_adaptive_avg_pool2d_nhwc(self):
        ...
        for test_case in test_cases:
            ...
            ops_under_test = {
                ...
                "nn.quantized.functional": torch.ao.nn.quantized.functional.adaptive_avg_pool2d,
    ...
    def test_adaptive_avg_pool(self):
        ...
        for test_case in test_cases:
            ...
            for dim in dim_to_check:
                ...
                # Run reference on int_repr + round to avoid double rounding error.
                ref_op = getattr(
                    torch.nn.functional, "adaptive_avg_pool{}d".format(dim)
                )
                ...
                ops_under_test = {
                    ...
                    "nn.quantized.functional": getattr(
                        ...
                        "adaptive_avg_pool{}d".format(dim),
                    ),
    ...
    def test_adaptive_avg_pool3d_ndhwc(self):
        ...
        for test_case in test_cases:
            ...
            ops_under_test = {
                ...
                "nn.quantized.functional": torch.ao.nn.quantized.functional.adaptive_avg_pool3d,
    ...
    def test_interpolate(self, X, size, mode, scale_factor, align_corners, nhwc_layout):
        ...
        for name, op in ops_under_test.items():
            ...
            self.assertEqual(
                ...
                atol=1.0,
                rtol=0,
                msg="{} results are off: qX_hat={} X_ref={}".format(
                    name, qX_hat.int_repr(), X_ref
                ),
                exact_dtype=False,
            )
            self.assertEqual(
...
class TestQuantizedLinear(TestCase):
    ...
    def test_qlinear_with_input_q_dq_qweight_dq_output_fp32(
        ...
        for dtype in dtypes:
            ...
            if dtype == torch.qint8 or qengine_is_onednn():
                ...
                W_zps = np.round(np.random.rand(output_channels) * 100 - 50).astype(
                    np.int
                )
            # when using symmetric quantization
            # special restriction for xnnpack fully connected op weight
    ...
    @unittest.skipIf(not TEST_CUDNN, "cudnn is not enabled.")
    @unittest.skip(
        "Local only - currently the qlinear_cudnn op is bulid "
        "with USE_EXPERIMENTAL_CUDNN_V8_API, we can enable the test "
        "after it is built by default"
    )
...
class TestQuantizedEmbeddingOps(TestCase):
    def _test_embedding_bag_unpack_impl(
        ...
        conversion_op = (
            ...
            else "HalfFloatToFused8BitRowwiseQuantized"
        )
        ...
        if bit_rate == 4:
            conversion_op = (
                ...
                if data_type == torch.float32
                else "HalfToFused4BitRowwiseQuantized"
            )
            ...
            conversion_op = (
                ...
                if data_type == torch.float32
                else "HalfToFused2BitRowwiseQuantized"
            )
        ...
        def get_c2_weights(weights, engine_str):
            ...
            if bit_rate == 4 or bit_rate == 2:
                workspace.RunOperatorOnce(
                    core.CreateOperator(
                        reverse_conversion_op,
                        ["quantized_weights"],
                        ["dequantized_weights"],
                    )
                )
                dequantized_data = torch.from_numpy(
                    workspace.FetchBlob("dequantized_weights")
                )
            else:
        ...
        np.testing.assert_allclose(
            w_packed.numpy(), w_packed_c2.numpy(), atol=1e-6, rtol=1e-6
        )
        # Compare unpacked weights against C2
        np.testing.assert_allclose(
            w_unpacked.numpy(), w_unpacked_c2.numpy(), atol=1e-6, rtol=1e-6
        )
    ...
    def test_embedding_bag_byte_unpack(
        self, num_embeddings, embedding_dim, num_batches, data_type
    ):
    ...
    def test_embedding_bag_4bit_unpack(
        self, num_embeddings, embedding_dim, optimized_qparams, data_type
    ):
    ...
    def test_embedding_bag_2bit_unpack(
        self, num_embeddings, embedding_dim, optimized_qparams, data_type
    ):
...
class TestQuantizedConv(TestCase):
    ...
    @unittest.skipIf(not TEST_CUDNN, "cudnn is not enabled.")
    @unittest.skip(
        "Local only - currently the qconv1d_cudnn op is bulid "
        "with USE_EXPERIMENTAL_CUDNN_V8_API, we can enable the test "
        "after it is built by default"
    )
        ...
        for shape, memory_format in test_cases:
            ...
            with override_quantized_engine("qnnpack"):
                ...
                self.assertEqual(
                    qY,
                    qY_hat,
                    msg="QNNPACK TanH failed (FP ref), memory_format {}".format(
                        memory_format
                    ),
                )
                self.assertEqual(
                    qYserver,
                    qY_hat,
                    msg="QNNPACK TanH failed (FBGEMM ref), memory_format {}".format(
                        memory_format
                    ),
                )
        ...
        for shape, memory_format in test_cases:
            ...
            with override_quantized_engine("qnnpack"):
                ...
                self.assertEqual(
                    qY,
                    qY_hat,
                    msg="QNNPACK Sigmoid failed (FP ref), memory_format {}".format(
                        memory_format
                    ),
                )
                self.assertEqual(
                    qYserver,
                    qY_hat,
                    msg="QNNPACK Sigmoid failed (FBGEMM ref), memory_format {}".format(
                        memory_format
                    ),
                )
...
class TestComparatorOps(TestCase):
    ...
    def test_compare_tensor_tensor(self, A, B):
        ...
        for op in ops_under_test:
            ...
            self.assertEqual(
                result_ref, result, msg="'tensor.{}(tensor)'' failed".format(op)
            )
    ...
    def test_compare_tensor_scalar(self, A, b):
        ...
        for op in ops_under_test_reversible:
            ...
            self.assertEqual(
                result_ref, result, msg="'scalar.{}(tensor)'' failed".format(op)
            )
