...
def upload_onnx_model(model_name, zoo_dir, backup=False, only_local=False):
    ...
    print(
        "Uploading {} ({} MB) to s3 cloud...".format(
            abs_file_name, float(file_size) / 1024 / 1024
        )
    )
    ...
    transfer.upload_file(
        abs_file_name,
        "download.onnx",
        "models/latest/{}".format(rel_file_name),
...
def download_onnx_model(model_name, zoo_dir, use_cache=True, only_local=False):
    ...
    try:
        ...
        print(
            "Failed to download/backup data for ONNX model {}: {}".format(model_name, e)
        )
...
def caffe2_to_onnx(caffe2_model_name, caffe2_model_dir):
    ...
    print(
        "Converting Caffe2 model {} in {} to ONNX format".format(
            caffe2_model_name, caffe2_model_dir
        )
    )
...
if __name__ == "__main__":
    ...
    for onnx_model_name in model_mapping:
        ...
        print(
            "####### Processing ONNX model {} ({} in Caffe2) #######".format(
                onnx_model_name, c2_model_name
            )
        )
        ...
        while os.path.exists(
            os.path.join(onnx_model_dir, "test_data_set_{}".format(starting_index))
        ):
            starting_index += 1
