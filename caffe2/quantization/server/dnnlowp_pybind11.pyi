...
def ObserveMinMaxOfOutput(
    min_max_file_name: str, dump_freq: int = -1, delimiter: str = " "
) -> None: ...
def ObserveHistogramOfOutput(
    out_file_name: str,
    dump_freq: int = -1,
    mul_nets: bool = False,
    op_filter: str = "",
...
def DumpOutputColumnMaxHistogramFile(
    ob: OutputColumnMaxHistogramNetObserver,
) -> None: ...
def AddOutputColumnMaxHistogramObserver(
    net_name: str,
    out_file_name: str,
    observe_column_max_for_blobs: List[str],
    dump_freq: int = -1,
    bin_nums: int = 16,
    mul_nets: bool = False,
    delimiter: str = " ",
...
def RegisterQuantizationParams(
    min_max_file_name: str, is_weight: bool = False, qparams_output_file_name: str = ""
) -> None: ...
def RegisterQuantizationParamsWithHistogram(
    histogram_file_name: str,
    is_weight: bool = False,
    qparams_output_file_name: str = "",
) -> None: ...
def AddRegisterQuantizationParamsWithHistogramObserver(
    net_name: str,
    histogram_file_name: str,
    is_weight: bool = False,
    qparams_output_file_name: str = "",
) -> RegisterQuantizationParamsWithHistogramNetObserver: ...
def AddScaleZeroOffsetArgumentsWithHistogram(
    net_def_bytes: bytes, histogram_file_name: str
) -> bytes: ...
...
def ChooseStaticQuantizationParams(
    min: float,
    max: float,
    bins: List[int],
...
def CreateInt8QuantSchemeBlob(
    quant_scheme_blob_name: str, quantization_kind: str, preserve_sparsity: bool
) -> None: ...
def CreateInt8QuantParamsBlob(
    quant_param_blob_name: str, scale: float, zero_point: int
) -> None: ...
