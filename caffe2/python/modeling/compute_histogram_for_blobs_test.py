...
class ComputeHistogramForBlobsTest(unittest.TestCase):
    def histogram(self, X, lower_bound=0.0, upper_bound=1.0, num_buckets=20):
        assert X.ndim == 2, "this test assume 2d array,  but X.ndim is {0}".format(
            X.ndim
        )
        N, M = X.shape
    ...
    def test_compute_histogram_for_blobs(self):
        ...
        upper_bound = 0.8
        accumulate = False
        net_modifier = ComputeHistogramForBlobs(
            blobs=["fc1_w", "fc2_w"],
        ...
        cur_hist, acc_hist = self.histogram(
            fc1_w,
            lower_bound=lower_bound,
            upper_bound=upper_bound,
            num_buckets=num_buckets,
        ...
        self.assertAlmostEqual(
            np.linalg.norm(fc1_w_curr_normalized_hist - cur_hist), 0.0, delta=1e-5
        )
