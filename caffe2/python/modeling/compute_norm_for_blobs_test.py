...
class ComputeNormForBlobsTest(unittest.TestCase):
    def test_compute_norm_for_blobs(self):
        ...
        self.assertAlmostEqual(fc1_w_l2_norm[0], np.linalg.norm(fc1_w) ** 2, delta=1e-5)
    ...
    def test_compute_l1_norm_for_blobs(self):
        ...
        self.assertAlmostEqual(fc1_w_l1_norm[0], np.sum(np.abs(fc1_w)), delta=1e-5)
    ...
    def test_compute_l1_averaged_norm_for_blobs(self):
        ...
        self.assertAlmostEqual(
            fc1_w_averaged_l1_norm[0], np.sum(np.abs(fc1_w)) / fc1_w.size, delta=1e-5
        )
    ...
    def test_compute_norm_row_index_for_blobs(self):
        ...
        fc1_w = workspace.FetchBlob("fc1_w")
        fc1_w_row_1_averaged_l2_norm = workspace.FetchBlob(
            "fc1_w_row_1_averaged_l2_norm"
        )

        self.assertEqual(fc1_w_row_1_averaged_l2_norm.size, 1)
        self.assertAlmostEqual(
            fc1_w_row_1_averaged_l2_norm[0],
