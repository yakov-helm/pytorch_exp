...
class ComputeHistogramForBlobs(NetModifier):
    ...
    def modify_net(
        ...
        for blob_name in self._blobs:
            ...
            blob_float = net.Cast(
                blob, net.NextScopedBlob(prefix=blob + "_float"), to=core.DataType.FLOAT
            )
            ...
            normalized_hist = net.NormalizeL1(
                hist, net.NextScopedBlob(prefix=blob + self._field_name_suffix)
            )
            ...
            if modify_output_record:
                ...
                output_scalar = schema.Scalar(
                    (np.float32, (self._num_buckets + 2,)), normalized_hist
                )
