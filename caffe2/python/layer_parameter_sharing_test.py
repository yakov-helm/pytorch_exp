...
class ParameterSharingTest(LayersTestCase):
    def test_layer_parameter_name(self):
        ...
        with scope.NameScope("global_scope"):
            ...
            with scope.NameScope("nested_scope"):
                ...
                self.assertEqual(
                    self.model.layers[-1].w, "global_scope/nested_scope/fc_auto_0/w"
                )
                self.assertEqual(
                    fc3_output(), "global_scope/nested_scope/fc_auto_0/output"
                )
        ...
        with scope.NameScope("global_scope"):
            with ParameterSharing({"fc_auto_0": "fc"}):
                ...
                with self.assertRaisesRegex(ValueError, "Got inconsistent shapes .*"):
                    self.model.FC(
                        self.model.input_feature_schema.float_features, output_dims + 1
                    )
    ...
    def test_layer_duplicated_parameter_init(self):
        ...
        self.assertEqual(
            sorted(op_outputs), ["global_scope/shared_fc/b", "global_scope/shared_fc/w"]
        )
