...
class BlobReference:
    ...
    def __dir__(self):
        ...
        return sorted(
            set(chain(dir(type(self)), self.__dict__.keys(), additional_methods))
        )
        ...
        for name, input_generators in sparse_generators.items():
            for version, generators in input_generators.items():
                if len(generators) == 1:
                    ...
                    assert (
                        dev_1 == dev_2
                    ), "Unequal devices for sparse generators: " "{} and {}".format(
                        dev_1, dev_2
                    )
...
class Net:
    ...
    def ClonePartial(self, name, inputs, outputs, remap_funcs=None):
        ...
        new_in = [blob_mapping[i] for i in input_names.keys()] + list(undef_blobs)
...
class ExecutionStep:
    ...
    def get_all_attributes(self, name):
        ...
        """
        return [
            attr for net in self._net_dict.values() for attr in net.get_attributes(name)
        ]
