...
def load_normalized_test_case(
    input_shape, test_image, pagelocked_buffer, normalization_hint
):
...
class Test_PT_ONNX_TRT(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(
            os.path.abspath(os.path.join(data_path, "class_labels.txt")), "r"
        ) as f:
    ...
    def _test_model(self, model_name, input_shape=(3, 224, 224), normalization_hint=0):
        ...
        with self.build_engine_onnx(onnx_name) as engine:
            ...
            with engine.create_execution_context() as context:
                err_count = 0
                for index, f in enumerate(self.image_files):
                    test_case = load_normalized_test_case(
                        input_shape, f, h_input, normalization_hint
                    )
                    cuda.memcpy_htod_async(d_input, h_input, stream)

                    context.execute_async_v2(
                        bindings=[d_input, d_output], stream_handle=stream.handle
                    )
                    ...
                    if (
                        "_".join(pred.split())
                        not in os.path.splitext(os.path.basename(test_case))[0]
                    ):
