...
class TestExtractPredictorNet(test_util.TestCase):
    @unittest.skipIf(
        "ImageInput" not in workspace.RegisteredOperators(), "Needs OpenCV"
    )
    ...
    def _test_op(
        ...
        if isinstance(in_option, list):
            assert len(in_option) == len(
                input_dev
            ), "Length of input device option should match" "{} vs. {}".format(
                in_option, input_dev
            )
        ...
        if isinstance(out_option, list):
            assert len(out_option) == len(
                output_dev
            ), "Length of output device option should match" "{} vs. {}".format(
                out_option, output_dev
            )
...
class TestConstructionFromProto(test_util.TestCase):
    def test_inplace_construction(self):
        ...
        self.assertEqual(
            len(n_copied.Proto().external_input), len(proto.external_input) - 1
        )
