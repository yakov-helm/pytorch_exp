...
class TestCaffe2Basic(TestCase):
    ...
    def test_check_arguments(self):
        ...
        bad_node_def = make_node(
            "Add", inputs=["X", "Y"], outputs=["Z"], foo=42, bar=56
        )
        with self.assertRaisesRegex(
            RuntimeError, "Don't know how to map unexpected argument (foo|bar)"
        ):
    ...
    def test_dynamicslice_4inputs_graph(self):
        ...
        prepared = c2.prepare(model_def)
        output = prepared.run(
            inputs=[np.array(x), np.array(start), np.array(end), np.array(axes)]
        )
    ...
    def test_elementwiselinear(self):
        ...
        ws, c2_outputs = c2_native_run_net(
            init_net=None, predict_net=predict_net, inputs=[X, W, B]
        )
    ...
    def test_initializer(self):
        ...
        graph_def = make_graph(
            ...
            outputs=[make_tensor_value_info("W3", onnx.TensorProto.FLOAT, (2, 2))],
            initializer=[
                make_tensor(
                    "weight",
    ...
    def test_reducemean(self):
        ...
        ws, c2_outputs = c2_native_run_net(
            init_net=None, predict_net=predict_net, inputs=[X]
        )

        onnx_model = c2_onnx.caffe2_net_to_onnx_model(
            predict_net=predict_net,
            value_info={"X": (onnx.mapping.NP_TYPE_TO_TENSOR_TYPE[X.dtype], X.shape)},
        )
    ...
    def test_gemm(self):
        ...
        node_def = make_node(
            "Gemm", ["A", "B", "C"], ["Y"], alpha=alpha, beta=beta, broadcast=1
        )
        ...
        node_def = make_node(
            "Gemm", ["A", "B", "C"], ["Y"], alpha=alpha, beta=beta, transB=1
        )
        output = c2.run_node(node_def, [A, B, C], opset_version=7)
        np.testing.assert_almost_equal(
            output["Y"], alpha * np.dot(A, np.transpose(B)) + beta * C
        )
    ...
    def test_gemm_conversion(self):
        ...
        node_def_broadcast = make_node(
            "Gemm", ["A", "B", "C"], ["Y"], alpha=2.0, beta=3.0, broadcast=1
        )
        node_def_transpose_b = make_node(
            "Gemm", ["A", "B", "C"], ["Y"], alpha=2.0, beta=3.0, transB=1
        )

        node_def_transpose_b_broadcast = make_node(
            "Gemm", ["A", "B", "C"], ["Y"], alpha=2.0, beta=3.0, transB=1, broadcast=1
        )
        ...
        _, op_strs = backend.convert_node(
            ...
            [
                make_tensor_value_info(
                    "A", onnx.TensorProto.FLOAT, (3, 2)
                ).SerializeToString(),
                make_tensor_value_info(
                    "B", onnx.TensorProto.FLOAT, (2, 3)
                ).SerializeToString(),
                make_tensor_value_info(
                    "C", onnx.TensorProto.FLOAT, (3, 3)
                ).SerializeToString(),
            ],
            6,
        )
    ...
    def test_concat(self):
        ...
        for i in range(2):
            ...
            ws, c2_outputs = c2_native_run_net(
                init_net=None, predict_net=predict_net, inputs=[I0, I1]
            )
            onnx_model = c2_onnx.caffe2_net_to_onnx_model(
                predict_net=predict_net,
                value_info={
    ...
    def _test_net(self, net_name, input_blob_dims=(1, 3, 224, 224), decimal=7):
        ...
        try:
            (
                ...
                value_info,
                debug_str,
            ) = self.model_downloader.get_c2_model_dbg(net_name)
        ...
        inputs = [data]
        _, c2_outputs = c2_native_run_net(
            c2_init_net, c2_predict_net, inputs, debug_str
        )
        del _
