...
class TestConversion(TestCase):
    ...
    def test_caffe2_to_onnx(self):
        ...
        init_model.net.GivenTensorFill(
            [], "X", shape=[2, 2], values=np.zeros((2, 2)).flatten().astype(float)
        )
    ...
    def test_caffe2_to_onnx_value_info(self):
        ...
        self.assertRaisesRegex(
            Exception, "value info", self._run_command, caffe2_to_onnx, args
        )
    ...
    def test_onnx_to_caffe2(self):
        ...
        self.assertEqual(
            set(sum([list(init_op.output) for init_op in caffe2_init_net.op], [])),
            {"W"},
        )
    ...
    def test_onnx_to_caffe2_zipfile(self):
        ...
        graph_def = helper.make_graph(
            ...
            initializer=[
                helper.make_tensor(
                    "W", TensorProto.FLOAT, [3, 2], W.tobytes(), raw=True
                )
            ],
        )
        model_def = helper.make_model(graph_def, producer_name="onnx-to-caffe2-test")
    ...
    def test_onnx_to_caffe2_if(self):
        ...
        false_nodes = [
            helper.make_node(
                "Slice", ["X"], ["Y"], axes=[0, 1], starts=[0, 0], ends=[2, 2]
            )
        ]
        nodes = self._make_fake_if_op(
            true_nodes, false_nodes, [(TensorProto.FLOAT, (2, 2), "Y")]
        )
        X = np.random.rand(2, 3).astype(np.float32)
        W = np.random.rand(3, 2).flatten().astype(np.float32)
        graph_def = helper.make_graph(
        ...
        model_def = helper.make_model(
            graph_def, producer_name="onnx-to-caffe2-test", opset_imports=[onnx_id]
        )
    ...
    def test_onnx_to_caffe2_loop(self):
        ...
        graph_def = helper.make_graph(
            ...
            [helper.make_tensor_value_info("Y", TensorProto.FLOAT, (2, 2))],
            initializer=[
                helper.make_tensor("W", TensorProto.FLOAT, [2, 2], W.tolist())
            ],
        )
        model_def = helper.make_model(graph_def, producer_name="onnx-to-caffe2-test")
        Y = X
    ...
    def test_convert_end2end(self):
        ...
        for _ in range(5):
            ...
            np.testing.assert_almost_equal(
                c2.run_model(onnx_model, {onnx_model.graph.input[0].name: x_val}),
                [y_val],
            )
            ...
            np.testing.assert_almost_equal(
                c2_native_run_net(
                    init_net=init_net, predict_net=predict_net, inputs={x: x_val}
                )[1],
                [y_val],
            )
