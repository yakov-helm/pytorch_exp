...
def convertAttributeProto(onnx_arg):
    ...
    if onnx_arg.HasField("f"):
        ...
        return Caffe2Backend._graph_to_net(
            onnx_arg.g, Caffe2Backend._known_opset_version
        )
        ...
        for g in onnx_arg.graphs:
            retval.append(
                Caffe2Backend._graph_to_net(g, Caffe2Backend._known_opset_version)
            )
        return retval
    else:
...
class Caffe2Backend(Backend):
    ...
    _per_op_renamed_attrs = {
        ...
        "Unsqueeze": {"axes": "dims"},
        "Transpose": {"perm": "axes"},
        "Upsample": {"mode": "", "scales": ""},
        "ConvTranspose": {"output_padding": "adjs"},
        "Selu": {"gamma": "scale"},
        "If": {"then_branch": "then_net", "else_branch": "else_net"},
        "RandomUniform": {"low": "min", "high": "max"},
    }
    ...
    def run_node(
        ...
        super(Caffe2Backend, cls).run_node(
            node,
            inputs,
            device=device,
        ...
        with core.DeviceScope(device_option):  # temporary!
            ...
            ops_str = cbackend.convert_node(
                node.SerializeToString(), value_infos, opset_version
            )
    ...
    @classmethod
    def _rnn_reform_weights(
        cls, reforms, name, hidden_size, init_net, gates, reorder_indices
    ):
        for name_from, name_to, do_concat, extra_dims in reforms:
            ...
            if do_concat:
                ...
                init_net.Concat(
                    ...
                    ["%s/%s" % (name, name_to), cls.dummy_name()],
                    axis=0,
                )

    @classmethod
    def _make_rnn_direction(
        cls,
        input_blob,
        B,
        W,
        ...
        if direction_offset == 1:
            if sequence_lens is not None:
                ...
                batch_size = pred_mh.net.Slice(
                    input_shape, name + "/batch_size_slice", starts=[1], ends=[2]
                )
                seq_len = pred_mh.net.Slice(
                    input_shape, name + "/seq_len_slice", starts=[0], ends=[1]
                )
                dummy_sequence_lens = pred_mh.net.Tile(
                    [seq_len, batch_size], name + "/dummy_sequence_lens", axis=0
                )
                pred_mh.net.Reshape(
                    dummy_sequence_lens,
    ...
    def _create_rnn_variant(cls, init_model, pred_model, n, opset_version):
        ...
        ), "cannot convert RNNs without access to the full model"
        assert (
            pred_model is not None
        ), "cannot convert RNNs without access to the full model"
        ...
        if n.op_type == "RNN":
            ...
            def make_cell(*args, **kwargs):
                return gru_cell.GRU(
                    *args, linear_before_reset=linear_before_reset, **kwargs
                )
    ...
    def _create_control_op(cls, init_model, pred_model, n, opset_version):
        ...
        node = cls._common_onnx_node_to_caffe2_op(
            init_model, pred_model, n, opset_version
        )
        ...
        while_op.arg.extend(
            [
                caffe2.python.utils.MakeArgument(
                    "num_loop_carried_deps", num_loop_carried_deps
                )
            ]
        )
    ...
    def _substitute_raw_value(cls, tp, raw_values_dict):
        if tp.HasField("raw_data") and tp.raw_data == bytes(b"__EXTERNAL"):
            if tp.name not in raw_values_dict:
                raise RuntimeError(
                    "TensorProto for value {} referenced raw data but it was not found!".format(
                        tp.name
                    )
                )
            else:
    ...
    def optimize_onnx(input, init=False, predict=False):
        ...
        try:
            ...
            warnings.warn(
                "OptimizerWarning: optimizer module not found in ONNX version {}".format(
                    onnx.__version__
                )
            )
        ...
        for imp in model.opset_import:
            if not imp.HasField("domain") or imp.domain == "":
                ...
                if imp.version > cls._known_opset_version:
                    warnings.warn(
                        "This version of onnx-caffe2 targets ONNX operator set version {}, but the model we are trying to import uses version {}.  We will try to import it anyway, but if the model uses operators which had BC-breaking changes in the intervening versions, import will fail.".format(
                            cls._known_opset_version, imp.version
                        )
                    )
            else:
    ...
    def _onnx_node_to_caffe2_op(cls, init_model, pred_model, node_def, opset_version):
        ...
        if cbackend.support_onnx_import(node_def.op_type):
            ...
            op_strs = cbackend.convert_node(
                node_def.SerializeToString(), value_infos, opset_version
            )
            init_ops = []
            for s in op_strs[0]:
                op = caffe2_pb2.OperatorDef()
    ...
    @classmethod
    def _common_onnx_node_to_caffe2_op(
        cls, init_model, pred_model, onnx_node, opset_version
    ):
        """
        ...
        if broken_version <= opset_version:
            raise ValueError(
                "Don't know how to translate op {} in ONNX operator set v{} (I only support prior to v{})".format(
        ...
        def kmap(k):
            if (
                onnx_op_type in cls._per_op_renamed_attrs
    ...
    @classmethod
    def _onnx_model_to_caffe2_net(
        cls, onnx_model, device, opset_version, include_initializers
    ):
        device_option = get_device_option(Device(device))
        ...
        try:
            ...
            warnings.warn(
                "ShapeInferenceWarning: utils module not found in ONNX version {}".format(
                    onnx.__version__
                )
            )
        ...
        try:
            ...
            warnings.warn(
                "OptimizerWarning: onnxoptimizer module not installed. "
        ...
        if include_initializers:
            init_net.op.extend(
                cls._create_tensor_filling_op(tp) for tp in onnx_model.graph.initializer
            )
    ...
    @classmethod
    def onnx_graph_to_caffe2_net(
        cls, model, device="CPU", opset_version=_known_opset_version
    ):
        return cls._onnx_model_to_caffe2_net(
            model, device=device, opset_version=opset_version, include_initializers=True
        )
    ...
    def is_compatible(cls, model, device="CPU", **kwargs):
        if hasattr(super(Caffe2Backend, cls), "is_compatible") and callable(
            super(Caffe2Backend, cls).is_compatible
        ):
