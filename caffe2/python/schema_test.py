...
class TestDB(unittest.TestCase):
    ...
    def testRawTuple(self):
        s = schema.RawTuple(2)
        self.assertEqual(
            s, schema.Struct(("field_0", schema.Scalar()), ("field_1", schema.Scalar()))
        )
    ...
    def testPreservesMetadata(self):
        s = schema.Struct(
            ...
            (
                "b",
                schema.Scalar(np.int32, metadata=schema.Metadata(categorical_limit=5)),
            ),
            (
                "c",
                schema.List(
                    schema.Scalar(
                        np.int32, metadata=schema.Metadata(categorical_limit=6)
                    )
                ),
            ),
        )
        ...
        sv = schema.from_blob_list(
            s, [np.array([3.4]), np.array([2]), np.array([3]), np.array([1, 2, 3])]
        )
    ...
    def testStructNestedAddition(self):
        s1 = schema.Struct(
            ("a", schema.Scalar()),
            ("b", schema.Struct(("c", schema.Scalar()))),
        )
