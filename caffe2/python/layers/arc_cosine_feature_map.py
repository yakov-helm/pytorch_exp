...
class ArcCosineFeatureMap(ModelLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert self.input_dims >= 1, (
            "Expected input dimensions >= 1, got %s" % self.input_dims
        )

        if initialize_output_schema:
            self.output_schema = schema.Scalar(
                (np.float32, (output_dims,)), model.net.NextScopedBlob(name + "_output")
            )
        ...
        assert isinstance(self.s, int), "Expected s to be type int, got type %s" % type(
            self.s
        )
        ...
        if set_weight_as_global_constant:
            w_init = np.random.normal(
                scale=self.stddev, size=(self.output_dims, self.input_dims)
            )
            b_init = np.random.uniform(
                low=-0.5 * self.stddev, high=0.5 * self.stddev, size=self.output_dims
            )
            self.random_w = self.model.add_global_constant(
                name=self.name + "_fixed_rand_W", array=w_init
            )
            self.random_b = self.model.add_global_constant(
                name=self.name + "_fixed_rand_b", array=b_init
            )
        else:
    ...
    def _initialize_params(
        self, w_name, b_name, w_init=None, b_init=None, w_optim=None, b_optim=None
    ):
        """
        ...
        w_init = (
            w_init if w_init else ("GaussianFill", {"mean": 0.0, "std": self.stddev})
        )
        ...
        b_init = (
            b_init
            if b_init
            else ("UniformFill", {"min": -0.5 * self.stddev, "max": 0.5 * self.stddev})
        )
        b_optim = b_optim if b_optim else self.model.NoOptim

        w_param = self.create_param(
            param_name=w_name,
    ...
    def _heaviside_with_power(self, net, input_features, output_blob, s):
        ...
        """
        if s == 0:
            softsign_features = net.Softsign(
                [input_features], net.NextScopedBlob("softsign")
            )
            ...
            pow_features = net.Pow(
                [input_features], net.NextScopedBlob("pow_rand"), exponent=float(s - 1)
            )
    ...
    def add_ops(self, net):
        ...
        random_features = net.FC(
            input_blob + [self.random_w, self.random_b],
            net.NextScopedBlob("random_features"),
        )
        # Process random features
        self._heaviside_with_power(
            net, random_features, self.output_schema.field_blobs(), self.s
        )
