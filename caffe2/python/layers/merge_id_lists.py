...
class MergeIdLists(ModelLayer):
    ...
    def __init__(self, model, input_record, name="merged"):
        ...
        assert all(
            schema.equal_schemas(x, IdList) for x in input_record
        ), "Inputs to MergeIdLists should all be IdLists."

        assert all(
            record.items.metadata is not None for record in self.input_record
    ...
    def add_ops(self, net):
        return net.MergeIdLists(
            self.input_record.field_blobs(), self.output_schema.field_blobs()
        )
