...
class SamplingTrain(ModelLayer):
    def __init__(
        ...
        **kwargs,
        ...
        assert isinstance(
            input_record.indices, schema.Scalar
        ...
        self._prediction_layer = layer_class(
            model, input_record.input, output_dims=output_dims, **kwargs
        )
    ...
    def add_train_ops(self, net):
        for full_blob, sampled_blob in zip(
            self._prediction_layer.param_blobs, self._prediction_layer.train_param_blobs
        ):
