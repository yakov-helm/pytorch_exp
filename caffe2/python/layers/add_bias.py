...
class AddBias(ModelLayer):
    def __init__(
        self, model, input_record, bias_init=None, bias_optim=None, name="add_bias"
    ):
        super().__init__(model, name, input_record)
        assert isinstance(input_record, schema.Scalar), "Incorrect input type"
        assert (
            len(input_record.field_type().shape) > 0
        ), "AddBias expects limited dimensions of the input tensor"

        input_dims = input_record.field_type().shape[0]
        assert input_dims > 0, "AddBias expects input dimensions > 0, got {}".format(
            input_dims
        )
    ...
    def add_ops(self, net):
        net.Add(
            self.input_record.field_blobs() + [self.b],
