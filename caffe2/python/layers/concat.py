...
class Concat(ModelLayer):
    ...
    def __init__(
        self, model, input_record, axis=1, add_axis=0, name="concat", **kwargs
    ):
        ...
        ), "Incorrect input type. Expected Struct, but received: {0}".format(
            input_record
        )
        ...
        for field_name, field_type in input_record.fields.items():
            ...
            ), "Incorrect input type for {}. Expected Scalar, but got: {}".format(
        ...
        for shape in shapes:
            ...
            assert (
                shape == shapes[0]
            ), "Shapes {0} and {1} are not compatible for Concat".format(
                shape, shapes[0]
            )
