...
class SparseLookup(ModelLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert isinstance(inner_shape, list) or isinstance(
            inner_shape, tuple
        ), "Unexpected type for inner_shape, expected list or tuple, got {0} for {1}".format(
            type(inner_shape), self.sparse_key
        )
        ...
        if self.trainer_version == "fp16":
            assert (
                self.reducer in self._fp16_compatible_reducers or use_external_weights
            ), (
        ...
        self.w = self.create_param(
            ...
            ps_param=LayerPsParam(
                sparse_key=self.sparse_key, average_length=avg_length
            ),
    ...
    def _gather_wrapper(self, net, version, in_indices, out):
        ...
        if version == "fp32":
            ...
            gathered_scale_bias = net.Gather(
                [self.scale_bias, in_indices], "gathered_scale_bias"
            )
            ...
            raise "Unsupported version of operators in SparseLookup " + "layer: {0} for sparse feature {1}".format(
    ...
    def _sparse_lengths_weighted_reducer(
        ...
        if version in ["fp32", "fp16"]:
            ...
            raise "Unsupported version of operator in SparseLookUp " + "layer: {0} for sparse feature {1}".format(
                version, self.sparse_key
            )
    ...
    def _add_ops_id_list(self, net, version):
        assert (
            self.reducer in self._id_list_supported_reducers
        ...
        if self.reducer in ["Sum", "Mean", "WeightedSum", "WeightedMean"]:
            ...
            self._sparse_lengths_weighted_reducer(
                self.input_record.items(), sqrt_weight, "WeightedSum", net, version
            )

        elif self.reducer == "None":
            # Gather operator will gather the embedding for each id of
            # each IdList.
            self._gather_wrapper(
                net,
                version,
    ...
    def _add_ops_id_score_list(self, net, version):
        assert (
            self.reducer in self._id_score_list_supported_reducers
        ...
        if self.reducer in ["WeightedSum", "WeightedMean"]:
            ...
            self.reducer in ["PositionWeighted", "RecencyWeighted"]
            or self.use_external_weights
        ):
            self._sparse_lengths_weighted_reducer(
                self.input_record.keys(),
            ...
            # each IdList.
            self._gather_wrapper(
                net, version, self.input_record.keys(), self.output_schema.field_blobs()
            )
        else:
            raise "Only Sum, Mean, None are supported for IdScoreList input." + "Trying to create with {} for sparse feature {}".format(
                self.reducer, self.sparse_key
            )
