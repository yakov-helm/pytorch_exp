...
class PositionWeighted(ModelLayer):
    def __init__(self, model, input_record, weight_optim=None, name="position_weights"):
        ...
        max_length = (
            length_metadata.categorical_limit if length_metadata is not None else None
        )
    ...
    def add_ops(self, net):
        inc_seq = net.LengthsRangeFill(
            [self.input_record.lengths()], self.input_record.lengths() + "_pos_w_seq"
        )

        net.Gather(
            [self.pos_w, inc_seq], self.output_schema.position_weights.field_blobs()
        )
