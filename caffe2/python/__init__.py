...
try:
    ...
    warnings.warn(
        "Caffe2 support is not enabled in this PyTorch build. "
...
caffe2_pb2.COMPILE_TIME_MAX_DEVICE_TYPES = (
    caffe2_pb2.PROTO_COMPILE_TIME_MAX_DEVICE_TYPES
)

if sys.platform == "win32":
    is_conda = os.path.exists(os.path.join(sys.prefix, "conda-meta"))
    py_dll_path = os.path.join(os.path.dirname(sys.executable), "Library", "bin")
    th_root = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "torch"
    )
    ...
    spec = importlib.util.spec_from_file_location(
        "torch_version", os.path.join(th_root, "version.py")
    )
    ...
    dll_paths = list(
        filter(
            os.path.exists, [th_dll_path, py_dll_path, nvtoolsext_dll_path, cuda_path]
        )
    )
    ...
    for dll in dlls:
        ...
        if with_load_library_flags:
            ...
            if res is None and last_error != 126:
                ...
                err.strerror += (
                    ' Error loading "{}" or one of its dependencies.'.format(dll)
                )
                raise err
