...
class TestPipeline(TestCase):
    def test_dequeue_many(self):
        ...
        src_values = Struct(
            ("uid", np.array(range(N))), ("value", 0.1 * np.array(range(N)))
        )
        expected_dst = Struct(
            ("uid", 2 * np.array(range(N))), ("value", np.array(N * [0.0]))
        )
        ...
        with TaskGroup() as tg:
            out1 = pipe(
                src_ds.reader(),
                output=Queue(capacity=11, num_dequeue_records=NUM_DEQUEUE_RECORDS),
                processor=proc1,
            )
