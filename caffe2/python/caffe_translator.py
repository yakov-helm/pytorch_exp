...
def _RemoveLegacyPad(net, net_params, input_dims):
    ...
    for i in range(len(net.op)):
        op_def = net.op[i]
        if re.match(r"^(Conv|ConvTranspose|MaxPool|AveragePool)(\dD)?$", op_def.type):
...
class TranslatorRegistry:
    ...
    def TranslateLayer(cls, layer, pretrained_blobs, is_test, **kwargs):
        try:
            ...
            raise KeyError(
                "No translator registered for layer: %s yet." % str(layer)
            ) from e
...
def TranslateConvNd(layer, pretrained_blobs, is_test, **kwargs):
    ...
    AddArgument(
        caffe_op, "kernels", [param.kernel_depth, param.kernel_size, param.kernel_size]
    )
    AddArgument(
        caffe_op, "strides", [param.temporal_stride, param.stride, param.stride]
    )
    temporal_pad = 0
    spatial_pad = 0
    ...
    if len(pretrained_blobs) == 2:
        ...
        params.append(
            utils.NumpyArrayToCaffe2Tensor(pretrained_blobs[1].flatten(), output + "_b")
        )
...
def TranslateCrop(layer, pretrained_blobs, is_test, **kwargs):
    ...
    assert (
        len(offsets) == 1
    ), "Caffe Translator for Crop only works for offset \
    of 1 for now"
    for _ in range(axis):
        starts.append(0)
        ends.append(-1)
...
def TranslateInnerProduct(layer, pretrained_blobs, is_test, **kwargs):
    ...
    weight = utils.NumpyArrayToCaffe2Tensor(
        pretrained_blobs[0].reshape(-1, pretrained_blobs[0].shape[-1]), output + "_w"
    )
...
def TranslateSoftmaxWithLoss(layer, pretrained_blobs, is_test, **kwargs):
    softmax_op = core.CreateOperator(
        "Softmax", [layer.bottom[0]], layer.bottom[0] + "_translator_autogen_softmax"
    )
...
def TranslateBatchNorm(layer, pretrained_blobs, is_test, **kwargs):
    ...
    if pretrained_blobs[2][0] != 0:
        mean = utils.NumpyArrayToCaffe2Tensor(
            (1.0 / pretrained_blobs[2][0]) * pretrained_blobs[0], output + "_mean"
        )
        var = utils.NumpyArrayToCaffe2Tensor(
            (1.0 / pretrained_blobs[2][0]) * pretrained_blobs[1], output + "_var"
        )
    else:
    ...
    if len(pretrained_blobs) > 3:
        ...
        bias = utils.NumpyArrayToCaffe2Tensor(
            pretrained_blobs[4].flatten(), output + "_bias"
        )
    else:
        ...
        bias = utils.NumpyArrayToCaffe2Tensor(
            np.zeros_like(pretrained_blobs[2]), output + "_bias"
        )
...
def TranslateScale(layer, pretrained_blobs, is_test, **kwargs):
    ...
    if len(mul_op.input) == 1:
        ...
        weights = []
        weights.append(
            utils.NumpyArrayToCaffe2Tensor(pretrained_blobs[0].flatten(), mul_op_param)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--init_net", help="Caffe2 initialization net.", default="init_net.pb"
    )
    parser.add_argument(
        "--predict_net", help="Caffe2 prediction net.", default="predict_net.pb"
    )
    parser.add_argument(
        "--remove_legacy_pad",
    ...
    parser.add_argument(
        "--input_dims", help="Dimension of input blob", nargs="+", type=int, default=[]
    )
