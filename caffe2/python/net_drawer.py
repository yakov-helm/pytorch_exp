    ...
    for i, step in enumerate(steps):
        ...
        if parallel:
            ...
            if len(step.substep) > kMaxParallelSteps:
                ellipsis = pydot.Node(
                    "{} more steps".format(len(step.substep) - kMaxParallelSteps),
                    **OP_STYLE,
                )
                g.add_node(ellipsis)
...
def GetGraphInJson(operators_or_net, output_filepath):
    ...
    for op_id, op in enumerate(operators):
        ...
        nodes.append(
            {"id": op_node_id, "label": op_label, "op_id": op_id, "type": "op"}
        )
        for input_name in op.input:
...
def main():
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="The input protobuf file."
    )
    parser.add_argument(
        "--output_prefix",
        type=str,
