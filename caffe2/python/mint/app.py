...
def visualize_print_log(filename):
    ...
    chart_name = os.path.splitext(os.path.basename(filename))[0]
    chart = nvd3.lineChart(
        name=chart_name + "_log_chart", height=args.chart_height, y_axis_format=".03g"
    )
    if args.sample < 0:
        step = max(data.shape[0] / -args.sample, 1)
    else:
    ...
    if data.shape[1] == 1:
        ...
        chart.add_serie(
            x=xdata[:trunc_size], y=running_mat.min(axis=1), name="running_min"
        )
        chart.add_serie(
            x=xdata[:trunc_size], y=running_mat.max(axis=1), name="running_max"
        )
        ...
        for i in range(0, min(data.shape[1], args.max_curves)):
            # data should have 4 dimensions.
            chart.add_serie(
                x=xdata, y=data[xdata, i], name="{}[{}]".format(chart_name, i)
            )
...
def visualize_file(filename):
    ...
    if filename.endswith("summary"):
        ...
        return flask.jsonify(result="Unsupport file: {}".format(filename), script="")
...
def main(argv):
    ...
    parser.add_argument(
        "-p", "--port", type=int, default=5000, help="The flask port to use."
    )
    parser.add_argument(
        "-r",
        "--root",
    ...
    parser.add_argument(
        "--chart_height", type=int, default=300, help="The chart height for nvd3."
    )
    parser.add_argument(
        "-s",
        "--sample",
