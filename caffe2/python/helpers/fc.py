    ...
    **kwargs,
    ...
    if model.init_params:
        compress_lb = model.param_init_net.ConstantFill(
            [], blob_out + "_lb", shape=[1], value=comp_lb
        )
        ...
        mask = model.param_init_net.ConstantFill(
            [], blob_out + "_m", shape=[dim_out, dim_in], value=1.0
        )
        ag_dw = model.param_init_net.__getattr__(mask_init[0])(
            [], blob_out + "_ag_dw", shape=[dim_out, dim_in], **mask_init[1]
        )
        ...
        mask_seq = model.param_init_net.__getattr__(mask_init[0])(
            [], blob_out + "_mask_seq", shape=[dim_out, dim_in], **mask_init[1]
        )
        thres = model.param_init_net.ConstantFill(
            [], blob_out + "_thres", shape=[1], value=threshold
        )
    else:
    ...
    if need_compress_rate:
        return model.net.FC_Prune(
            [blob_in, weight, mask, bias, ag_dw, mask_seq, thres, compress_lb],
            [blob_out, compress_rate],
            **kwargs,
        )
    else:
        return model.net.FC_Prune(
...
def fc_sparse(model, blob_in, blob_out, w_csr, iw, jw, bias, **kwargs):
    ...
    model.AddParameter(bias)
    return model.net.FC_Sparse([blob_in, w_csr, iw, jw, bias], blob_out, **kwargs)
