...
def OpAlmostEqual(op_a, op_b, ignore_fields=None):
    ...
    assert all(
        isinstance(f, str) for f in ignore_fields
...
def MakeArgument(key, value):
    ...
    elif iterable and all(type(v) in [bool, int, np.int_] for v in value):
        argument.ints.extend(v.item() if type(v) is np.int_ else v for v in value)
    elif iterable and all(isinstance(v, bytes) or isinstance(v, str) for v in value):
...
def BuildUniqueMutexIter(init_net, net, iter=None, iter_mutex=None, iter_val=0):
    ...
    if not init_net.BlobIsDefined(iter):
        ...
        with core.DeviceScope(
            core.DeviceOption(caffe2_pb2.CPU, extra_info=["device_type_override:cpu"])
        ):
