...
class ShuffleNetV2Builder:
    ...
    def create(self):
        ...
        self.prev_blob = brew.max_pool(
            self.model, self.prev_blob, "stage1_pool", kernel=3, stride=2
        )

        # adds stage#{2,3,4}; see table 5 of the ShufflenetV2 paper.
        for idx, (out_channels, n_repeats) in enumerate(
            zip(self.output_channels[1:4], self.stage_repeats)
        ):
    ...
    def add_spatial_ds_unit(self, prefix, in_channels, out_channels, stride=2):
        ...
        if self.detection:
            left = self.add_detection_unit(
                left, prefix + "_left_detection", in_channels, in_channels
            )

        left = self.add_dwconv3x3_bn(left, prefix + "left_dwconv", in_channels, stride)
        left = self.add_conv1x1_bn(
            left, prefix + "_left_conv1", in_channels, out_channels
        )

        if self.detection:
            right = self.add_detection_unit(
                right, prefix + "_right_detection", in_channels, in_channels
            )

        right = self.add_conv1x1_bn(
            right, prefix + "_right_conv1", in_channels, out_channels
        )
        right = self.add_dwconv3x3_bn(
            right, prefix + "_right_dwconv", out_channels, stride
        )
        right = self.add_conv1x1_bn(
            right, prefix + "_right_conv2", out_channels, out_channels
        )

        self.prev_blob = brew.concat(self.model, [right, left], prefix + "_concat")
        self.prev_blob = self.model.net.ChannelShuffle(
            self.prev_blob, prefix + "_ch_shuffle", group=self.num_groups, kernel=1
        )
    ...
    def add_basic_unit(self, prefix, in_channels, stride=1):
        ...
        right = self.add_conv1x1_bn(
            right, prefix + "_right_conv1", in_channels, in_channels
        )
        right = self.add_dwconv3x3_bn(
            right, prefix + "_right_dwconv", in_channels, stride
        )
        right = self.add_conv1x1_bn(
            right, prefix + "_right_conv2", in_channels, in_channels
        )
    ...
    def add_detection_unit(
        self, prev_blob, prefix, in_channels, out_channels, kernel=3, pad=1
    ):
        out_blob = brew.conv(
            self.model,
            prev_blob,
...
def create_shufflenet(
    ...
    builder = ShuffleNetV2Builder(
        model, data, num_input_channels, num_labels, is_test=is_test
    )
    builder.create()
