...
class TestOperators(hu.HypothesisTestCase):
    ...
    @given(
        X=hu.arrays(dims=[5, 2], elements=hu.floats(min_value=1.0, max_value=10.0)),
        **hu.gcs_cpu_only,
    )
    @settings(deadline=1000)
    ...
    @given(
        input_tensor=hu.arrays(
            dims=[10], elements=hu.floats(allow_nan=False, allow_infinity=False)
        ),
        **hu.gcs,
    )
    @settings(deadline=10000)
    ...
    @given(
        input_tensor=hu.arrays(
            dims=[10], elements=hu.floats(min_value=-10, max_value=10)
        ),
        **hu.gcs,
    )
    @settings(deadline=10000)
    ...
    @given(
        input_tensor=hu.arrays(
            dims=[10], elements=hu.floats(min_value=1, max_value=10000)
        ),
        **hu.gcs_cpu_only,
    )
    @settings(deadline=10000)
    ...
    def test_tt_sls_gradientop(self, gc, dc):
        ...
        indices = np.array(
            [[0, 0, 0], [1, 0, 0], [2, 0, 0], [1, 0, 0], [4, 0, 0]], np.int64
        )
    ...
    def test_tt_sls_repro(self, gc, dc):
        ...
        golden = np.array(
            [[0] * 64, [768] * 64, [0] * 64, [0] * 64, [512] * 64, [0] * 64, [0] * 64]
        )
    ...
    @given(
        a=hu.tensor(elements=hu.floats(allow_nan=True)),
    ...
    def test_elman_recurrent_network(self, t, n, d):
        ...
        backward_link_internal, backward_link_external, backward_link_offset = zip(
            *backward_links
        )
    ...
    @given(
        inp=_dtypes().flatmap(
            lambda dt: _tensor_and_indices(elements=hu.elements_of_type(dt), dtype=dt)
        ),
        **hu.gcs,
    )
    @settings(deadline=10000)
    def test_sparse_to_dense(self, inp, gc, dc):
        ...
        self.assertReferenceChecks(
            gc, op_noshapeinfer, [I, X], sparse_to_dense_noshapeinfer
        )
