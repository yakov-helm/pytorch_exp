...
class TestLayers(LayersTestCase):
    ...
    def testAddLoss(self):
        ...
        self.model.add_loss(
            schema.Scalar(dtype=np.float32, blob=core.BlobReference("loss_blob_1")),
            "addLoss",
        )
        ...
        self.model.add_loss(
            schema.Struct(
                (
                    "structName",
                    schema.Scalar(
                        dtype=np.float32, blob=core.BlobReference("loss_blob_2")
                    ),
                )
            ),
            "addLoss",
        ...
        assert (
            core.BlobReference("loss_blob_in_tuple_0") in self.model.loss.field_blobs()
        )
        assert (
            core.BlobReference("loss_blob_in_tuple_1") in self.model.loss.field_blobs()
        )
    ...
    def testFCWithBootstrap(self):
        ...
        assert (
            core.BlobReference("fc_with_bootstrap/bootstrap_iteration_0/indices")
            == fc_with_bootstrap[0].field_blobs()[0]
        )
        assert (
            core.BlobReference("fc_with_bootstrap/bootstrap_iteration_0/preds")
            == fc_with_bootstrap[1].field_blobs()[0]
        )
        assert (
            core.BlobReference("fc_with_bootstrap/bootstrap_iteration_1/indices")
            == fc_with_bootstrap[2].field_blobs()[0]
        )
        assert (
            core.BlobReference("fc_with_bootstrap/bootstrap_iteration_1/preds")
            == fc_with_bootstrap[3].field_blobs()[0]
        )
    ...
    def testFCwithAxis2(self):
        ...
        input_record = self.new_record(
            schema.Struct(
                (
                    "history_sequence",
                    schema.Scalar((np.float32, (max_length, input_dim))),
                ),
            )
        )
    ...
    def testFCTransposed(self):
        ...
        fc_transposed_out = self.model.FC(
            input_record.history_sequence, output_dim, axis=2, transposed=True
        )
        self.model.output_schema = fc_transposed_out
        self.assertEqual(
            schema.Scalar((np.float32, (max_length, output_dim))), fc_transposed_out
        )
    ...
    def testSparseLookupSumPoolingWithEviction(self):
        ...
        embedding_after_pooling = self.model.SparseLookup(
            record.sparse.sparse_feature_0,
            [embedding_dim],
            "Sum",
            weight_init=("ConstantFill", {"value": 1.0}),
        )
    ...
    def testSparseFeatureHashIdList(self, use_hashing, modulo, use_divide_mod, divisor):
        record = schema.NewRecord(
            ...
            schema.List(
                schema.Scalar(
                    np.int64, metadata=schema.Metadata(categorical_limit=60000)
                )
            ),
        )
    ...
    def testSparseFeatureHashIdScoreList(self, use_hashing, modulo):
        ...
        output_schema = self.model.SparseFeatureHash(
            record, modulo=modulo, use_hashing=use_hashing
        )
    ...
    def testPairwiseSimilarityWithAllEmbeddings(self):
        ...
        record = schema.NewRecord(
            self.model.net,
            schema.Struct(
                ("all_embeddings", schema.Scalar(((np.float32, (N, embedding_dim))))),
            ),
        )
    ...
    def testPairwiseSimilarityWithXandYEmbeddings(self):
        ...
        record = schema.NewRecord(
            self.model.net,
            schema.Struct(
                ("x_embeddings", schema.Scalar(((np.float32, (5, embedding_dim))))),
                ("y_embeddings", schema.Scalar(((np.float32, (6, embedding_dim))))),
            ),
        )
    ...
    def testPairwiseSimilarityIncorrectInput(self):
        ...
        record = schema.NewRecord(
            self.model.net, schema.Struct(("all_embeddings", schema.List(np.float32)))
        )
        with self.assertRaises(AssertionError):
            self.model.PairwiseSimilarity(record, 25)
    ...
    def testSamplingTrain(self):
        ...
        sub_spec = OpSpec(
            "Sub", [sampled_fc.field_blobs()[0], None], sampled_fc.field_blobs()
        )

        train_ops = self.assertNetContainOps(
            train_net, [gather_w_spec, gather_b_spec, train_fc_spec, log_spec, sub_spec]
        )
    ...
    def testBPRLoss(self):
        ...
        np.testing.assert_array_almost_equal(
            np.array(1.24386, dtype=np.float32), result
        )
    ...
    def testReservoirSamplingWithID(self, X, num_to_collect):
        ...
        (
            train_init_net,
            train_net,
        ) = layer_model_instantiator.generate_training_nets_forward_only(self.model)
        workspace.RunNetOnce(train_init_net)
    ...
    def testUniformSampling(self):
        ...
        np.testing.assert_almost_equal(
            np.array(
                [float(num_samples) / num_elements] * num_samples, dtype=np.float32
            ),
            sampling_prob,
        )
    ...
    def testGatherRecord(self):
        ...
        np.testing.assert_array_equal(
            np.concatenate([lengths[i : i + 1] for i in indices]), gathered_lengths
        )
        ...
        np.testing.assert_array_equal(
            np.concatenate(
                [items_lengths[offsets[i] : offsets[i] + lengths[i]] for i in indices]
            ),
        ...
        np.testing.assert_array_equal(
            np.concatenate(
                [
                    items_items[
                        nested_offsets[i] : nested_offsets[i] + nested_lengths[i]
                    ]
                    for i in indices
                ]
            ),
    ...
    def testMapToRange(self):
        ...
        schema.FeedRecord(
            input_record, [np.array([10, 3, 20, 99, 15, 11, 3, 11], dtype=np.int32)]
        )
        workspace.RunNetOnce(train_init_net)
        workspace.RunNetOnce(train_net)
        ...
        schema.FeedRecord(
            input_record, [np.array([10, 3, 23, 35, 60, 15, 10, 15], dtype=np.int32)]
        )
        workspace.RunNetOnce(train_net)
        indices = workspace.FetchBlob(indices_blob())
        np.testing.assert_array_equal(
        ...
        schema.FeedRecord(
            input_record, [np.array([10, 3, 23, 35, 60, 15, 200], dtype=np.int32)]
        )
        workspace.RunNetOnce(eval_net)
        indices = workspace.FetchBlob(indices_blob())
        np.testing.assert_array_equal(
        ...
        schema.FeedRecord(
            input_record, [np.array([10, 3, 23, 15, 101, 115], dtype=np.int32)]
        )
        workspace.RunNetOnce(eval_net)
        indices = workspace.FetchBlob(indices_blob())
        np.testing.assert_array_equal(
    ...
    def testSelectRecordByContext(self):
        ...
        _, train_net = layer_model_instantiator.generate_training_nets_forward_only(
            self.model
        )
        workspace.RunNetOnce(train_net)
    ...
    def testFunctionalLayerHelper(self):
        ...
        normalized = self.model.Sub(
            schema.Tuple(self.model.input_feature_schema.float_features, mean),
            1,
            broadcast=1,
        )
        # Attach metadata to one of the outputs and use it in FC
    ...
    def testPropagateRequestOnly(self):
        ...
        input_record2 = (
            self.new_record(
                schema.Struct(("input4", schema.Scalar((np.float32, (100,)))))
            )
            + input_record
        )
    ...
    def testDropout(self, X, dropout_for_eval):
        ...
        with_d_spec = OpSpec(
            "Dropout", [input_blob], [output_blob, None], {"is_test": 0, "ratio": 0.5}
        )

        without_d_spec = OpSpec(
            "Dropout", [input_blob], [output_blob, None], {"is_test": 1, "ratio": 0.5}
        )
    ...
    def testMergeIdListsLayer(self, num_inputs, batch_size):
        ...
        input_schema = schema.Tuple(
            *[
                schema.List(
                    schema.Scalar(
                        dtype=np.int64, metadata=schema.Metadata(categorical_limit=20)
                    )
                )
                for _ in range(num_inputs)
            ]
    ...
    def testRandomFourierFeatures(self, batch_size, input_dims, output_dims, bandwidth):
        ...
        schema.FeedRecord(input_record, [X])
        input_blob = input_record.field_blobs()[0]
        rff_output = self.model.RandomFourierFeatures(
            input_record, output_dims, bandwidth
        )
        self.model.output_schema = schema.Struct()
        ...
        fc_spec = OpSpec(
            "FC", [input_blob, init_ops[0].output[0], init_ops[1].output[0]], None
        )
    ...
    def testArcCosineFeatureMap(
        ...
        def _arc_cosine_hypothesis_test(ac_output, X, W, b, s):
            ...
            if s > 0:
                h_rand_features = np.piecewise(
                    x_rand, [x_rand <= 0, x_rand > 0], [0, 1]
                )
            else:
                h_rand_features = np.piecewise(
                    x_rand, [x_rand <= 0, x_rand > 0], [0, lambda x: x / (1 + x)]
                )
    ...
    def testSemiRandomFeatures(
        ...
        def _semi_random_hypothesis_test(
            srf_output, X_full, X_random, rand_w, rand_b, s
        ):
            """
            Runs hypothesis test for Semi Random Features layer.
        ...
        if use_struct_input:
            X_random = np.random.normal(size=(batch_size, input_dims)).astype(
                np.float32
            )
        ...
        if set_weight_as_global_constant:
            ...
            fc_learned_spec = OpSpec(
                "FC", [None, init_ops[0].output[0], init_ops[1].output[0]], None
            )
        else:
            init_ops = self._test_net(train_init_net, init_ops_list)
            ...
            fc_learned_spec = OpSpec(
                "FC", [None, init_ops[2].output[0], init_ops[3].output[0]], None
            )
        ...
        _semi_random_hypothesis_test(
            srf_output.full(), X_full, X_random, rand_w, rand_b, s
        )
    ...
    def testAdaptiveWeight(
        ...
        result = self.model.AdaptiveWeight(
            ...
            pos_optim_method=("log_barrier" if use_log_barrier else "pos_grad_proj"),
    ...
    def _testLabelSmooth(self, categories, binary_prob_label, bsz):
        ...
        if binary_prob_label:
            expected = np.array(
                [smooth_matrix[0] if x == 0.0 else smooth_matrix[1] for x in label_np]
            )
        else:
    ...
    def testBlobWeightedSum(self, num_inputs, batch_size, input_dim, seed):
        def get_blob_weighted_sum():
            ...
            for i in range(num_inputs):
                ...
                assert workspace.HasBlob(w_blob_name), "cannot fine blob {}".format(
                    w_blob_name
                )
            ...
            result = np.sum(
                [input_data[idx] * weights[idx] for idx in range(num_inputs)], axis=0
            )
            return result
    ...
    def testFeatureSparseToDenseGetAccessedFeatures(self):
        ...
        input_record = self.new_record(
            schema.Struct(
                ...
                (id_list_features_column, schema.Map(np.int32, schema.List(np.int64))),
                (
        ...
        self.assertEqual(len(self.model.layers), 1)
        self.assertEqual(
            self.model.layers[0].get_accessed_features(), expected_accessed_features
        )
    ...
    def test_get_key(self):
        ...
        def _is_id_score_list(input_record):
            return almost_equal_schemas(
                input_record, IdScoreList, check_field_types=False
            )
        ...
        id_list_record = schema.NewRecord(
            ...
            schema.List(
                schema.Scalar(
                    np.int64, metadata=schema.Metadata(categorical_limit=1000)
                )
            ),
        )

        self.assertEqual(
            get_key(id_list_record)(), old_get_sparse_key_logic(id_list_record)
        )
