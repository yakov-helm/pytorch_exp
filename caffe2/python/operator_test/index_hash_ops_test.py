...
class TestIndexHashOps(serial.SerializedTestCase):
    @given(
        indices=st.sampled_from([np.int32, np.int64]).flatmap(
            lambda dtype: hu.tensor(min_dim=1, max_dim=1, dtype=dtype)
        ),
    ...
    def test_index_hash_ops(self, indices, seed, modulo, gc, dc):
        ...
        op = core.CreateOperator(
            "IndexHash", ["indices"], ["hashed_indices"], seed=seed, modulo=modulo
        )
        ...
        # In-place update
        op = core.CreateOperator(
            "IndexHash", ["indices"], ["indices"], seed=seed, modulo=modulo
        )
