...
class TestElementwiseOps(hu.HypothesisTestCase):
    ...
    @given(
        X=hu.tensor(
            elements=hu.floats(min_value=0.1, max_value=10.0), dtype=np.float32
        ),
        inplace=st.booleans(),
        **hu.gcs,
    )
    ...
    @given(
        X=hu.tensor(
            elements=hu.floats(min_value=1.0, max_value=10.0), dtype=np.float32
        ),
    ...
    def test_div_legacy_grad(
        self, n: int, m: int, broadcast: bool, allow_broadcast_fastpath: bool, gc, dc
    ):
        op = core.CreateOperator(
            "DivGradient",
            ["B", "C", "dC"],
    ...
    @given(
        X=hu.tensor(elements=hu.floats(min_value=0.5, max_value=2), dtype=np.float32),
        inplace=st.booleans(),
        **hu.gcs,
    )
    @settings(deadline=10000)
