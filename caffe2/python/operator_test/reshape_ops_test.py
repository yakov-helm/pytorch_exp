...
class TestLengthsToShapeOps(TestCase):
    ...
    def test_reshape_ops(self):
        ...
        workspace.RunOperatorOnce(
            core.CreateOperator("Reshape", ["input", "shape"], ["output", "old_shape"])
        )
    ...
    def test_basic_reshape(self):
        ...
        _test_reshape_output_and_gradient(
            old_shape=(4, 2, 1), new_shape=(2, 4), arg_shape=False
        )

    def test_missing_dim(self):
        _test_reshape_output_and_gradient(old_shape=(4, 2, 1), new_shape=(-1, 8))
        _test_reshape_output_and_gradient(
            old_shape=(4, 2, 1), new_shape=(-1, 8), arg_shape=False
        )

    def test_in_place(self):
        _test_reshape_output_and_gradient(
            old_shape=(4, 2, 1), new_shape=(-1, 8), in_place=True
        )
        _test_reshape_output_and_gradient(
    ...
    def test_zero_dim_and_missing_dim(self):
        ...
        _test_reshape_output_and_gradient(
            old_shape=(4, 3, 2), new_shape=(-1, 0), expected_shape=(8, 3)
        )
        _test_reshape_output_and_gradient(
            old_shape=(4, 3, 2),
            new_shape=(-1, 0),
        ...
        _test_reshape_output_and_gradient(
            old_shape=(2, 0), new_shape=(-1, 0), expected_shape=(0, 0), arg_shape=False
        )
    ...
    def test_one_dim_and_empty_tensor(self):
        ...
        _test_reshape_output_and_gradient(
            old_shape=old_shape, new_shape=new_shape, expected_shape=expected_new_shape
        )
...
def _test_reshape_output_and_gradient(
    ...
    for device_opt in devices:
        with core.DeviceScope(device_opt):
            ...
            if arg_shape:
                out, _ = net.Reshape(
                    [blob_in], [blob_out, "old_shape"], shape=new_shape
                )
            else:
