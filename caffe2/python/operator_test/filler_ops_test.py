...
class TestFillerOperator(serial.SerializedTestCase):
    ...
    @given(
        shape=hu.dims().flatmap(
            lambda dims: hu.arrays(
                [dims], dtype=np.int64, elements=st.integers(min_value=0, max_value=20)
            )
        ),
    ...
    def test_uniform_int_fill_op_blob_input(self, shape, a, b, gc, dc):
        ...
        uniform_fill = net.UniformIntFill(
            [shape_blob, a_blob, b_blob], 1, input_as_shape=1
        )
    ...
    def test_fp16_uniformfill_op(self, min, range, emb_size, dim_size, gc, dc):
        ...
        for device_option in dc:
            ...
            assert workspace.RunOperatorOnce(
                op
            ), "Float16UniformFill op did not run successfully"
            ...
            self.assertAlmostEqual(
                np.mean(blob_out, dtype=np.float32), expected_mean, delta=0.1
            )
            self.assertAlmostEqual(
                np.var(blob_out, dtype=np.float32), expected_var, delta=0.1
            )
