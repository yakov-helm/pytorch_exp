...
class TestAdadelta(serial.SerializedTestCase):
    @staticmethod
    def ref_adadelta(
        param_in, mom_in, mom_delta_in, grad, lr, epsilon, decay, using_fp16=False
    ):
        ...
        new_grad = (
            np.sqrt(mom_delta_in_f32 + epsilon) / np.sqrt(mom_out + epsilon)
        ) * grad
    ...
    def test_sparse_adadelta(self, inputs, lr, epsilon, decay, gc, dc):
        ...
        def ref_sparse(
            param, moment, moment_delta, indices, grad, lr, decay, ref_using_fp16
        ):
