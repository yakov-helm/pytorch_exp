...
class TestFeatureMapsOps(TestCase):
    def test_merge_dense_feature_tensors(self):
        ...
        workspace.FeedBlob(
            "in1",
            np.array(
                [[11.1, 12.1, 13.1, 14.1], [11.2, 12.2, 13.2, 14.2]], dtype=np.float
            ),
        )
        workspace.FeedBlob(
            "in1_presence",
            np.array(
                [[True, False, False, True], [False, True, True, False]], dtype=np.bool
            ),
        )
        ...
        np.testing.assert_array_equal(
            workspace.FetchBlob("out_lengths"), np.array([2, 2], dtype=np.int32)
        )
        np.testing.assert_array_equal(
            workspace.FetchBlob("out_keys"), np.array([11, 14, 12, 13], dtype=np.int64)
        )
        np.testing.assert_array_equal(
