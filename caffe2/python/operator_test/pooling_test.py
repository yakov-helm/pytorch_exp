...
class TestPooling(hu.HypothesisTestCase):
    ...
    @given(
        ...
        op_type=st.sampled_from(
            ["MaxPool", "AveragePool", "LpPool", "MaxPool2D", "AveragePool2D"]
        ),
        **hu.gcs,
    )
    @settings(deadline=10000)
    def test_pooling_separate_stride_pad(
        self,
        stride_h,
        stride_w,
        pad_t,
    ...
    @given(
        ...
        order=st.sampled_from(["NCHW", "NHWC"]),
        op_type=st.sampled_from(
            ["MaxPool", "AveragePool", "MaxPool1D", "AveragePool1D"]
        ),
        **hu.gcs,
    )
    @settings(deadline=10000)
    ...
    @given(
        ...
        order=st.sampled_from(["NCHW", "NHWC"]),
        op_type=st.sampled_from(
            ["MaxPool", "AveragePool", "MaxPool3D", "AveragePool3D"]
        ),
    ...
    def test_pooling_3d(
        ...
        if hiputl.run_in_hip(gc, dc) and (
            workspace.GetHIPVersion() < 303 or order == "NHWC"
        ):
            assume(engine != "CUDNN")
        ...
        X = np.random.rand(batch_size, size, size, size, input_channels).astype(
            np.float32
        )
        if order == "NCHW":
            X = utils.NHWC2NCHW(X)
    ...
    def test_global_pooling_3d(
        self, kernel, size, input_channels, batch_size, order, op_type, engine, gc, dc
    ):
        # Currently MIOpen Pooling only supports pooling with NCHW order.
        if hiputl.run_in_hip(gc, dc) and (
            workspace.GetHIPVersion() < 303 or order == "NHWC"
    ...
    def test_pooling_with_index(
        self, stride, pad, kernel, size, input_channels, batch_size, gc, dc
    ):
        assume(pad < kernel)
        op = core.CreateOperator(
            "MaxPoolWithIndex",
    ...
    def test_global_pooling(
        self, size, input_channels, batch_size, order, op_type, engine, gc, dc
    ):
        # CuDNN 5 does not support deterministic max pooling.
    ...
    @given(
        op_type=st.sampled_from(["MaxPool", "MaxPoolND"]),
        dim=st.integers(1, 3),
        N=st.integers(1, 3),
        C=st.integers(1, 3),
        D=st.integers(3, 5),
    ...
    def test_max_pool_grad(
        self, op_type, dim, N, C, D, H, W, kernel, stride, pad, order, engine, gc, dc
    ):
        assume(pad < kernel)
    ...
    @given(
        op_type=st.sampled_from(["AveragePool", "AveragePoolND"]),
        dim=st.integers(1, 3),
        N=st.integers(1, 3),
        C=st.integers(1, 3),
        D=st.integers(3, 5),
