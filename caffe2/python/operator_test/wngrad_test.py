...
def wngrad_sparse_test_helper(parent_test, inputs, seq_b, lr, epsilon, engine, gc, dc):
    ...
    indices = np.random.choice(
        np.arange(grad.shape[0]), size=np.random.randint(grad.shape[0]), replace=False
    )
...
class TestWngrad(serial.SerializedTestCase):
    ...
    def test_wngrad_dense_output_effective_lr(self, inputs, seq_b, lr, epsilon, gc, dc):
        ...
        self.assertReferenceChecks(
            ...
            op,
            [param, seq_b, grad, lr],
            functools.partial(ref_wngrad, epsilon=epsilon, output_effective_lr=True),
        )
    ...
    def test_wngrad_dense_output_effective_lr_and_update(
        ...
        self.assertReferenceChecks(
            ...
            [param, seq_b, grad, lr],
            functools.partial(
                ref_wngrad, epsilon=epsilon, output_effective_lr_and_update=True
            ),
        )
