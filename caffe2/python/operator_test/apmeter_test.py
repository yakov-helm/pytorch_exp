...
class TestAPMeterOps(hu.HypothesisTestCase):
    @given(
        predictions=hu.arrays(
            dims=[10, 3],
            elements=hu.floats(
                allow_nan=False, allow_infinity=False, min_value=0.1, max_value=1
            ),
        ),
        labels=hu.arrays(
            dims=[10, 3], dtype=np.int32, elements=st.integers(min_value=0, max_value=1)
        ),
        **hu.gcs_cpu_only,
    )
    def test_average_precision(self, predictions, labels, gc, dc):
        ...
        self.assertReferenceChecks(
            device_option=gc, op=op, inputs=[predictions, labels], reference=op_ref
        )
