...
class TestActivations(serial.SerializedTestCase):
    ...
    def test_relu(self, X, in_place, engine, gc, dc):
        ...
        self.assertReferenceChecks(
            gc, op, [X], relu_ref, ensure_outputs_are_inferred=True
        )
    ...
    @unittest.skipIf(
        not workspace.has_gpu_support, "Relu for float16 can only run on GPU now."
    )
    @given(
        X=hu.tensor(dtype=np.float16),
        in_place=st.booleans(),
        engine=st.sampled_from(["", "CUDNN"]),
        **hu.gcs,
    )
    ...
    @serial.given(
        X=hu.tensor(elements=hu.floats(-3.0, 3.0)),
    ...
    def test_relu_n(self, X, n, in_place, gc, dc):
        ...
        self.assertReferenceChecks(
            gc, op, [X], relu_n_ref, ensure_outputs_are_inferred=True
        )
        self.assertDeviceChecks(dc, op, [X], [0])
        self.assertGradientChecks(
            gc, op, [X], 0, [0], stepsize=0.005, ensure_outputs_are_inferred=True
        )
    ...
    def test_elu(self, X, alpha, in_place, engine, gc, dc):
        ...
        self.assertReferenceChecks(
            gc, op, [X], elu_ref, ensure_outputs_are_inferred=True
        )
        self.assertDeviceChecks(dc, op, [X], [0])
        self.assertGradientChecks(
    ...
    @given(
        X=hu.tensor(min_dim=4, max_dim=4),
        alpha=hu.floats(min_value=0.1, max_value=2.0),
        inplace=st.booleans(),
        shared=st.booleans(),
    ...
    def test_prelu(self, X, alpha, inplace, shared, order, seed, gc, dc):
        np.random.seed(seed)
        W = np.random.randn(X.shape[1] if order == "NCHW" else X.shape[3]).astype(
            np.float32
        )
        ...
        op = core.CreateOperator(
            "PRelu", ["X", "W"], ["Y" if not inplace else "X"], alpha=alpha, order=order
        )
        self.assertReferenceChecks(
            gc, op, [X, W], prelu_ref, ensure_outputs_are_inferred=True
        )
        ...
        if not inplace:
            ...
            self.assertGradientChecks(
                gc, op, [X, W], 0, [0], stepsize=1e-2, ensure_outputs_are_inferred=True
            )
            # Gradient check wrt W
            self.assertGradientChecks(
                gc, op, [X, W], 1, [0], stepsize=1e-2, ensure_outputs_are_inferred=True
            )
    ...
    def test_leaky_relu(self, X, alpha, inplace, gc, dc):
        ...
        op = core.CreateOperator(
            "LeakyRelu", ["X"], ["Y" if not inplace else "X"], alpha=alpha
        )
        self.assertReferenceChecks(
            gc, op, [X], leaky_relu_ref, ensure_outputs_are_inferred=True
        )
        # Check over multiple devices
        self.assertDeviceChecks(dc, op, [X], [0])
    ...
    def test_gelu(self, X, fast_gelu, gc, dc):
        ...
        self.assertReferenceChecks(
            gc, op, [X], gelu_ref, threshold=tol, ensure_outputs_are_inferred=True
        )
        self.assertDeviceChecks(dc, op, [X], [0])
        self.assertGradientChecks(gc, op, [X], 0, [0], ensure_outputs_are_inferred=True)
