...
class TestResize(hu.HypothesisTestCase):
    @given(
        height_scale=st.floats(0.25, 4.0) | st.just(2.0),
    ...
    def test_nearest(
        ...
        def ref(X):
            ...
            output_h_idxs, output_w_idxs = np.meshgrid(
                np.arange(output_height), np.arange(output_width), indexing="ij"
            )

            input_h_idxs = np.minimum(output_h_idxs / height_scale, height - 1).astype(
                np.int32
            )
            input_w_idxs = np.minimum(output_w_idxs / width_scale, width - 1).astype(
                np.int32
            )
    ...
    def test_nearest_onnx(
        ...
        def ref(X, scales):
            ...
            input_h_idxs = np.minimum(output_h_idxs / scales[0], height - 1).astype(
                np.int32
            )
            input_w_idxs = np.minimum(output_w_idxs / scales[1], width - 1).astype(
                np.int32
            )
        ...
        self.assertGradientChecks(
            gc, op, [X, scales], 0, [0], stepsize=0.1, threshold=1e-2
        )
