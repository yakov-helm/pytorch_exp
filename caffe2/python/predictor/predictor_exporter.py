...
class PredictorExportMeta(
    ...
    def train_init_plan_name(self):
        plan_name = utils.get_comp_name(
            predictor_constants.TRAIN_INIT_PLAN_TYPE, self.name
        )
        return (
            self.trainer_prefix + "_" + plan_name if self.trainer_prefix else plan_name
        )
...
def _global_init_net(predictor_export_meta, db_type):
    ...
    if db_type != "manifold_db":
        net.Load(
            [predictor_constants.PREDICTOR_DBREADER], predictor_export_meta.parameters
        )
...
def get_meta_net_def(predictor_export_meta, ws=None, db_type=None):
    ...
    utils.AddNet(
        meta_net_def,
        predictor_export_meta.predict_init_name(),
        utils.create_predict_init_net(ws, predictor_export_meta),
    )
    utils.AddNet(
        meta_net_def,
        predictor_export_meta.global_init_name(),
        _global_init_net(predictor_export_meta, db_type),
    )
    utils.AddNet(
        meta_net_def,
        predictor_export_meta.predict_net_name(),
        utils.create_predict_net(predictor_export_meta),
    )
    utils.AddBlobs(
        meta_net_def,
        predictor_export_meta.parameters_name(),
        predictor_export_meta.parameters,
    )
    utils.AddBlobs(
        meta_net_def, predictor_export_meta.inputs_name(), predictor_export_meta.inputs
    )
    utils.AddBlobs(
        meta_net_def,
        predictor_export_meta.outputs_name(),
...
def save_to_db(
    db_type, db_destination, predictor_export_meta, use_ideep=False, *args, **kwargs
):
...
def load_from_db(filename, db_type, device_option=None, *args, **kwargs):
    ...
    assert workspace.RunOperatorOnce(create_db), "Failed to create db {}".format(
        filename
    )
