    ...
    def run_with(self, builder):
        with Cluster():
            ...
            for initial_epoch in range(1, num_epochs + 1):
                session, checkpoint = builder()
                JobRunner(job, checkpoint, resume_from_epoch=initial_epoch).train(
                    session
                )
                self.assertEqual(fetch_total(session), EXPECTED_TOTALS[-1])
        ...
        try:
            ...
            with Cluster():
                ...
                for node_id in range(num_nodes):
                    ...
                    self.assertEqual(
                        checkpoint.get_ckpt_db_name(node_name, epoch), expected_db_name
                    )
            shutil.rmtree(tmpdir)
            ...
            with Cluster():
                ...
                for epoch in range(1, 5):
                    self.assertTrue(
                        job_runner.load_blobs_from_checkpoints(
                            blob_names=model_blob_names, epoch=epoch, session=session
                        )
                    )
            ...
            for node_id in range(3):
                ...
                with Cluster():
                    ...
                    job_runner = JobRunner(
                        job, checkpoint, upload_task_group_builder=local_upload_builder
                    )
                    num_epochs = job_runner.train(session)
                    self.assertEqual(num_epochs, len(EXPECTED_TOTALS))
