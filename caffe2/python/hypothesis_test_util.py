...
def settings(*args, **kwargs):
    if "min_satisfying_examples" in kwargs and hypothesis.version.__version_info__ >= (
        3,
        56,
        0,
    ):
...
def tensor(
    min_dim=1, max_dim=4, dtype=np.float32, elements=None, unique=False, **kwargs
):
...
gcs_gpu_only = dict(
    gc=st.sampled_from(_gpu_do_list), dc=st.just(_gpu_do_list)
)  # CUDA or ROCm
gcs_no_hip = dict(
...
class HypothesisTestCase(test_util.TestCase):
    ...
    def _assertInferTensorChecks(
        ...
        try:
            ...
            if (
                os.getenv("CAFFE2_ASSERT_SHAPEINFERENCE") == "1"
                or ensure_output_is_inferred
            ):
                raise e
    ...
    def assertReferenceChecks(
        ...
        with temp_workspace():
            ...
            try:
                ...
                if (
                    os.getenv("CAFFE2_ASSERT_SHAPEINFERENCE") == "1"
                    or ensure_outputs_are_inferred
                ):
                    raise e
    ...
    def assertValidationChecks(
        ...
        with temp_workspace():
            ...
            if as_kwargs:
                validator(
                    **dict(zip(list(op.input) + list(op.output), inputs + outputs))
                )
            else:
