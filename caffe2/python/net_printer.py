...
class Visitor:
    ...
    def __call__(self, obj, *args, **kwargs):
        ...
        if Type not in self.__class__.visitors:
            raise TypeError(
                "%s: unsupported object type: %s" % (self.__class__.__name__, Type)
            )
...
def call(op, inputs=None, outputs=None, factor_prefixes=False):
    ...
    return (
        call
        if not outputs
        else "%s = %s" % (factor_prefix(outputs, factor_prefixes), call)
    )
...
def _get_step_context(step):
    ...
    if proto.num_concurrent_instances > 1:
        return (
            call("parallel", [("num_instances", proto.num_concurrent_instances)]),
            len(step.Substeps()) > 1,
        )
    ...
    with text.context(step_ctx):
        ...
        for substep in substeps:
            ...
            if sub_proto is not None and sub_proto.run_every_ms:
                substep_ctx = call(
                    "reporter", [str(substep), ("interval_ms", sub_proto.run_every_ms)]
                )
            elif do_substep:
                title = (
                    "workspace"
                    if sub_proto is not None and sub_proto.create_workspace
                    else "step"
                )
