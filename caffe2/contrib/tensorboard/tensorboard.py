...
@click.option(
    "--c2-netdef", type=click.Path(exists=True, dir_okay=False), multiple=True
)
...
def tensorboard_graphs(c2_netdef, tf_dir):
    ...
    graph_defs = [
        tb_exporter.nets_to_graph_def([parse_net_def(path)]) for path in c2_netdef
    ]
    events = [
        graph_def_to_event(i, graph_def)
