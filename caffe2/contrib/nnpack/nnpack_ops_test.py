...
class NNPackOpsTest(hu.HypothesisTestCase):
    ...
    def test_convolution_correctness(
        self, stride, pad, kernel, size, input_channels, batch_size, groups
    ):
        ...
        w = (
            np.random.rand(input_channels, output_channels, kernel, kernel).astype(
                np.float32
            )
            - 0.5
        )
        ...
        np.testing.assert_allclose(outputs[""], outputs["NNPACK"], atol=1e-4, rtol=1e-4)
    ...
    def test_timings(
        self, stride, pad, kernel, size, input_channels, output_channels, batch_size
    ):
        assume(stride <= kernel)
        X = (
            np.random.rand(batch_size, input_channels, size, size).astype(np.float32)
