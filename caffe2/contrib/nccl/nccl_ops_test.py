...
def benchmark(ws, net, warmups=5, iters=100):
    ...
    after = time.time()
    print(
        "Timing network, time taken per-iteration: {:.6f}ms".format(
...
class NCCLOpsTest(hu.HypothesisTestCase):
    ...
    @given(
        n=st.integers(min_value=2, max_value=workspace.NumGpuDevices()),
        m=st.integers(min_value=1, max_value=1000),
        root=st.integers(min_value=0, max_value=workspace.NumGpuDevices() - 1),
    )
    ...
    def test_nccl_reduce(self, n, m, root, in_place):
        ...
        op = core.CreateOperator(
            "NCCLReduce", inputs, inputs[root] if in_place else b"o", root=root
        )
        input_device_options = {n: gpu_device(i) for i, n in enumerate(inputs)}
    ...
    def _test_nccl_sync(self, n, m, iters, net_type):
        ...
        for i in range(n):
            net.ConstantFill(
                [], inputs[i], shape=[m], value=0.0, device_option=gpu_device(i)
            )
            net.ConstantFill(
                [], extra_inputs[i], shape=[m], value=1.0, device_option=gpu_device(i)
            )
            for _ in range(iters):
                net.Sum(
                    [inputs[i], extra_inputs[i]],
                    [inputs[i]],
    ...
    def test_timings(self):
        for n in range(2, workspace.NumGpuDevices()):
            for in_place in [False, True]:
                xs = [np.random.randn(1e7).astype(np.float32) for i in range(n)]
                inputs = [str("x_{}".format(i)) for i in range(n)]
                prefix = "" if in_place else "o"
                outputs = [str("{}x_{}".format(prefix, i)) for i in range(n)]
