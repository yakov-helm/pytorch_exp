...
def initOpts(opts):
    ...
    assert (
        opts["distributed"]["num_gpus"] > 0 or opts["distributed"]["num_cpus"] > 0
    ), "Need to specify num_gpus or num_cpus to decide which device to use."

    trainWithCPU = opts["distributed"]["num_gpus"] == 0
    num_xpus = (
        opts["distributed"]["num_cpus"]
        if trainWithCPU
        else opts["distributed"]["num_gpus"]
    )
    first_xpu = (
        opts["distributed"]["first_cpu_id"]
        if trainWithCPU
        else opts["distributed"]["first_gpu_id"]
    )
...
class AnyExpTrainer:
    ...
    def list_of_epochs(self):
        iter_end_point = min(
            self.opts["epoch_iter"]["num_epochs"],
            self.epoch + self.opts["epoch_iter"]["num_epochs_per_flow_schedule"],
        )
    ...
    def checkpoint(self, epoch):
        self.model_path = checkpoint.save_model_params(
            True,
            self.train_model,
            self.gen_checkpoint_path(True, epoch + 1),
            epoch + 1,
            self.opts,
            float("-inf"),
        )
    ...
    def prep_a_data_parallel_model(self, model, dataset, is_train):
        ...
        optimizer = (
            ...
            if self.gen_optimizer_fun is not None
            else None
        )
    ...
    def buildModelAndTrain(self, opts):
        ...
        log.info(
            "check self get_input_dataset methods: {}".format(
                inspect.getsource(self.get_input_dataset)
            )
        )
        log.info(
            "check self gen_input_builder_fun method: {}".format(
                inspect.getsource(self.gen_input_builder_fun)
            )
        )
        log.info(
            "check self gen_forward_pass_builder_fun method: {}".format(
        ...
        if self.gen_param_update_builder_fun is not None:
            ...
            log.info(
                "check self gen_optimizer_fun method: {}".format(
                    inspect.getsource(self.gen_optimizer_fun)
                )
            )
        log.info(
            "check self assembleAllOutputs method: {}".format(
                inspect.getsource(self.assembleAllOutputs)
            )
        )
        log.info(
            "check self prep_data_parallel_models method: {}".format(
                inspect.getsource(self.prep_data_parallel_models)
            )
        )
