...
def resnet_imagenet_create_model(model, data, labels, split, opts, dataset):
    ...
    assert (
        opts_depth in BLOCK_CONFIG.keys()
    ...
    loss_scale = (
        1.0 / opts["distributed"]["num_xpus"] / opts["distributed"]["num_shards"]
    )
    ...
    softmax, loss = model.SoftmaxWithLoss(
        [fc_blob, labels], ["softmax", "loss"], scale=loss_scale
    )
...
class ResNetModelHelper:
    ...
    def conv_bn_relu(
        ...
        bn_blob = self.conv_bn(
            blob_in, dim_in, dim_out, kernel, stride, prefix, group=group, pad=pad
        )
    ...
    def group_bottleneck_block(
        ...
        blob_out = self.conv_bn_relu(
            blob_out, dim_inner, dim_inner, 3, stride, prefix + "_branch2b", group=group
        )
        bn_blob = self.conv_bn(
    ...
    def residual_layer(
        ...
        for idx in range(num_blocks):
            ...
            blob_in = block_fn(
                blob_in, dim_in, dim_out, block_stride, block_prefix, dim_inner, group
            )
            dim_in = dim_out
