...
def init_model(self):
    train_model = cnn.CNNModelHelper(
        order="NCHW", name="resnet", use_cudnn=True, cudnn_exhaustive_search=False
    )
...
def fun_per_iter_b4RunNet(self, epoch, epoch_iter):
    ...
    for idx in range(
        ...
        self.opts["distributed"]["first_xpu_id"] + self.opts["distributed"]["num_xpus"],
    ):
        caffe2_pb2_device = (
            caffe2_pb2.CUDA
            if self.opts["distributed"]["device"] == "gpu"
            else caffe2_pb2.CPU
        )
