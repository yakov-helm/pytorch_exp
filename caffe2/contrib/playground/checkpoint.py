...
def initialize_params_from_file(
    ...
    num_xpus,
    opts,
    broadcast_computed_param=False,
    reset_epoch=False,
):
...
def initialize_master_xpu_model_params(model, weights_file, opts, reset_epoch):
    ...
    if model is not None:
        log.info(
            "initialize model parameters using weights file: {}".format(weights_file)
        )
        ...
        caffe2_pb2_DEVICE = (
            caffe2_pb2.CUDA
            if opts["distributed"]["device"] == "gpu"
            else caffe2_pb2.CPU
        )
        ...
        log.info(
            "Skip initializing model parameters from file: {}".format(weights_file)
        )
...
def broadcast_parameters(opts, model, num_xpus, broadcast_computed_param=False):
    ...
    caffe2_pb2_DEVICE = (
        caffe2_pb2.CUDA if opts["distributed"]["device"] == "gpu" else caffe2_pb2.CPU
    )
    for params in all_params:
        assert (
            len(params) % num_xpus == 0
...
def save_model_params(is_checkpoint, model, checkpoint_path, epoch, opts, best_metric):
    ...
    try:
        save_model_params_blob(model, checkpoint_path, epoch, opts, best_metric)
...
def save_model_params_blob(model, params_file, epoch, opts, best_metric):
    ...
    device = opts["distributed"]["device"]
    save_params = [
        str(param) for param in model.GetParams("{}_{}".format(device, root_xpu_id))
    ]
    save_computed_params = [
        str(param)
        for param in model.GetComputedParams("{}_{}".format(device, root_xpu_id))
    ]
    save_blobs = {}
    save_blobs["epoch"] = epoch
    save_blobs["best_metric"] = best_metric
    save_blobs["lr"] = workspace.FetchBlob("{}_{}/lr".format(device, root_xpu_id))
    for param in save_params + save_computed_params:
        ...
        if unscoped_blob_name not in save_blobs:
            save_blobs[unscoped_blob_name] = workspace.FetchBlob(scoped_blob_name)
            log.debug("{:s} -> {:s}".format(scoped_blob_name, unscoped_blob_name))
