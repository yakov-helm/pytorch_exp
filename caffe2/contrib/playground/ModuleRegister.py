...
def registerModuleMap(module_map):
    ...
    log.info(
        "ModuleRegister get modules from  ModuleMap content: {}".format(
            inspect.getsource(module_map)
        )
    )
...
def constructTrainerClass(myTrainerClass, opts):
    ...
    log.info(
        "ModuleRegister, myTrainerClass name is {}".format(myTrainerClass.__name__)
    )
    ...
    log.info(
        "ModuleRegister, myInitializeModelModule dir is {}".format(
            dir(myInitializeModelModule)
        )
    )
    ...
    log.info(
        "ModuleRegister, myInputModule {} dir is {}".format(
            opts["input"]["input_name_py"], myInputModule.__name__
        )
    )
    ...
    myTrainerClass.gen_param_update_builder_fun = (
        ...
        if myParamUpdateModule is not None
        else None
    )

    myOptimizerModule = getModule(opts["model"]["optimizer_py"])
    myTrainerClass.gen_optimizer_fun = (
        myOptimizerModule.gen_optimizer_fun if myOptimizerModule is not None else None
    )
    ...
    myTrainerClass.gen_rendezvous_ctx = (
        ...
        if myRendezvousModule is not None
        else None
    )
...
def overrideAdditionalMethods(myTrainerClass, opts):
    log.info(
        "B4 additional override myTrainerClass source {}".format(
            inspect.getsource(myTrainerClass)
        )
    )
    ...
    if myAdditionalOverride is not None:
        for funcName, funcValue in inspect.getmembers(
            myAdditionalOverride, inspect.isfunction
        ):
            setattr(myTrainerClass, funcName, funcValue)
    log.info(
        "Aft additional override myTrainerClass's source {}".format(
            inspect.getsource(myTrainerClass)
        )
    )
    return myTrainerClass
...
def getModule(moduleName):
    log.info(
        "get module {} from MODULE_MAPS content {}".format(moduleName, str(MODULE_MAPS))
    )
    myModule = None
    for ModuleMap in MODULE_MAPS:
        ...
        for name, obj in inspect.getmembers(ModuleMap):
            ...
            if name == moduleName:
                log.info(
                    "AnyExp get module {} with source:{}".format(
                        moduleName, inspect.getsource(obj)
                    )
                )
                myModule = obj
                return myModule
...
def getClassFromModule(moduleName, className):
    ...
    for ModuleMap in MODULE_MAPS:
        for name, obj in inspect.getmembers(ModuleMap):
            if name == moduleName:
                log.info(
                    "ModuleRegistry from module {} get class {} of source:{}".format(
                        moduleName, className, inspect.getsource(obj)
                    )
                )
