...
def instantiate_device_type_tests(
    ...
    if (
        ...
        and MPSTestBase not in test_bases
    ):
        test_bases.append(MPSTestBase)
        ...
        for op in self.op_list:
            ...
            for dtype in dtypes:
                ...
                try:
                    ...
                    print(
                        "Failed to instantiate {0} for op {1}!".format(
                            test_name, op.name
                        )
                    )
                    raise ex
...
class toleranceOverride:
    def __init__(self, d):
        ...
        for dtype, prec in d.items():
            assert isinstance(
                dtype, torch.dtype
            ), "toleranceOverride given unknown dtype {0}".format(dtype)
            assert isinstance(
                prec, tol
        ...
        def wrap_fn(self, *args, **kwargs):
            if self.device_type == "cuda":
                ...
                if (
                    ...
                    reason = "ROCm {0} is available but {1} required".format(
                        rocm_version_tuple, version
                    )
                    raise unittest.SkipTest(reason)
