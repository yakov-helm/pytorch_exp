...
def instantiate_parametrized_tests(generic_cls):
    ...
    for attr_name in tuple(dir(generic_cls)):
        ...
        def instantiate_test_helper(cls, name, test, param_kwargs):
            ...
            assert not hasattr(generic_cls, name), "Redefinition of test {0}".format(
                name
            )
...
class parametrize(_TestParametrizer):
    ...
    def _formatted_str_repr(self, name, value):
        ...
        # Can't use isinstance as it would cause a circular import
            ...
            value.__class__.__name__ == "OpInfo"
            or value.__class__.__name__ == "ModuleInfo"
        ):
    ...
    def _default_subtest_name(self, values):
        return "_".join(
            [self._formatted_str_repr(a, v) for a, v in zip(self.arg_names, values)]
        )
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
        if len(self.arg_names) == 0:
            ...
            if values is check_exhausted_iterator:
                raise ValueError(
                    "An empty arg_values was passed to @parametrize. "
...
def retry_shell(
    command, cwd=None, env=None, stdout=None, stderr=None, timeout=None, retries=1
):
    assert (
        retries >= 0
    ...
    if not IS_SANDCASTLE:
        ...
        for disabled_test, (issue_url, platforms) in disabled_tests_dict.items():
            ...
            if len(disable_test_parts) > 1:
                ...
                if (
                    test._testMethodName == disabled_test_name
                    or sanitized_test_method_name == disabled_test_name
        ...
        *,
        ...
        def random_sparse_compressed(n_compressed_dims, n_plain_dims, nnz):
            ...
            for i in range(n_compressed_dims):
                ...
                (
                    plain_indices[compressed_indices[i] : compressed_indices[i + 1]],
                    _,
                ) = torch.sort(
...
class TestGradients(TestCase):
    ...
    def _skip_helper(self, op, device, dtype):
        ...
        if op.name == "cat":
            self.skipTest(
                "TODO(whc) fix pre-existing bug with cat for newly added opinfo for empty+nonempty"
            )
