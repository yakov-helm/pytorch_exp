...
def get_broadcast_compatible_shape(tensor_shape):
    ...
    for i in np.random.choice(
        range(max_dim - trim_head), num_b_dims - trim_head, replace=False
    ):
        shape[i] = 1
    return shape
...
def random_topology_test(seed, *inp_tensor_list):
    ...
    while num_operations >= 0 or num_sets > 1:
        ...
        if DEBUG_PRINT:
            print(
                "iteration {0}, num_sets{1}, candidates {2}, tensor_list {3}, lh_index {4}, op_index {5}".format(
                    num_operations,
                    num_sets,
                    candidate,
                    len(tensor_list),
        ...
        if num_operations >= 0:
            ...
            if op_index < u_op_size:
                ...
                if DEBUG_PRINT:
                    print(
                        "binary, op_2_index {0}, rh_index ?{1}".format(
                            op_2_index, rh_index
                        )
                    )
        else:
        ...
        if rh_index is not None:
            ...
            out_tensor = binary_operations[op_index - u_op_size](
                tensor_list[lh_index], tensor_list[rh_index]
            )
...
def prepareInputTensorsToRandomTopoTest(
    seed, max_tensor_num, max_tensor_dim, max_tensor_size, debug_tensor, device, dtype
):
    ...
    if debug_tensor:
        ...
        for i in range(tensor_dim):
            size_i = np.random.randint(
                1, int(max_tensor_size / numel / (2 ** (tensor_dim - i)))
            )
    ...
    broadcasted_tensors_indices = np.random.choice(
        torch.arange(num_tensor), num_broadcasted_tensors, replace=False
    )
    ...
    for i in range(num_tensor):
        if i in broadcasted_tensors_indices:
            ...
            tensor_list.append(
                torch.randn(compatible_shape, device=device, dtype=dtype) * 100
            )
        else:
            tensor_list.append(
                torch.randn(tensor_shape, device=device, dtype=dtype) * 100
            )
...
def reproString(current_seed, args):
    ...
    if args.cpu:
        repro_str += " --cpu"
    repro_str += (
        " --max-num-tensor {0} --max-tensor-dim {1} --max-tensor-size {2}"
...
def runDefaultTestWithSeed(seed):
    ...
    seed_tensor, tensor_list = prepareInputTensorsToRandomTopoTest(
        seed, MAX_TENSOR, MAX_TENSOR_DIM, MAX_TENSOR_SIZE, DEBUG_TENSOR, DEVICE, DTYPE
    )
...
def runTest(seed, args):
    ...
    try:
        ...
        raise Exception(
            "Testing script failure with error message, repro by running:\n"
            f"\t{reproString(seed, args)}"
        ) from err
    try:
        traced_model = torch.jit.trace(
            random_topology_test, (seed_tensor, *tensor_list)
        )
        if DEBUG_PRINT:
        ...
        if DEBUG_PRINT:
            print(
                "optimized graph: ", traced_model.graph_for(seed_tensor, *tensor_list)
            )
        ...
        raise Exception(
            "cuda fuser gives wrong results, repro by running:\n"
            f"\t{reproString(seed, args)}"
        ) from err
    except Exception as err:
        raise Exception(
            "something in cuda fuser went wrong, repro by running:\n"
            f"\t{reproString(seed, args)}"
        ) from err
...
def parse_args():
    ...
    parser.add_argument(
        "--cuda-fuser", "--cuda_fuser", action="store_true", default=True
    )
    parser.add_argument(
        "--legacy-fuser", "--legacy_fuser", action="store_true", default=False
    )
    parser.add_argument(
        "--profiling-executor",
        "--profiling_executor",
    ...
    parser.add_argument(
        "--debug-print", "--debug_print", action="store_true", default=False
    )
    parser.add_argument(
        "--debug-tensor", "--debug_tensor", action="store_true", default=False
    )
    parser.add_argument(
        "--max-num-tensor", "--max_num_tensor", default=MAX_TENSOR, type=int
    )
    parser.add_argument(
        "--max-tensor-dim", "--max_tensor_dim", default=MAX_TENSOR_DIM, type=int
    )
    parser.add_argument(
        "--max-tensor-size", "--max_tensor_size", default=MAX_TENSOR_SIZE, type=int
    )
    parser.add_argument(
        "--depth-factor", "--depth-factor", default=GRAPH_FACTOR, type=int
    )
...
if __name__ == "__main__":
    ...
    if args.repro_run:
        ...
        if len(failing_repros) == 0:
            ...
            print(
                "{0} out of {1} tests failed;".format(
