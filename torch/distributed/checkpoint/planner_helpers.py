...
def _create_write_items_for_dtensor(fqn: str, tensor: DTensor) -> WriteItem:
    ...
    assert device_mesh.ndim == 1, "Only 1D DeviceMeshes can currently be handled."

    sizes = torch.Size(
        compute_local_shape(tensor.shape, device_mesh, tensor.placements)
    )
    offsets = torch.Size(
        compute_local_offset(tensor.shape, device_mesh, tensor.placements)
    )
