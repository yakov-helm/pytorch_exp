    ...
    if rocm_home is None:
        ...
        try:
            ...
            rocm_home = os.path.dirname(
                os.path.dirname(hipcc.decode(*SUBPROCESS_DECODE_ARGS).rstrip("\r\n"))
            )
...
def CUDAExtension(name, sources, *args, **kwargs):
    ...
    if dlink:
        ...
        if (torch.version.cuda is not None) and packaging.version.parse(
            torch.version.cuda
        ) >= packaging.version.parse("11.2"):
...
def _write_ninja_file(
    ...
    if with_cuda:
        ...
        if (
            has_cuda_version
            and packaging.version.parse(torch.version.cuda) >= required_cuda_version
        ):
