...
class _DataPipeType:
    ...
    def issubtype(self, other):
        ...
        raise TypeError(
            "Expected '_DataPipeType' or 'type', but found {}".format(type(other))
        )
...
class _DataPipeMeta(GenericMeta):
    ...
    def _getitem_(self, params):
        ...
        if not t.issubtype(self.type):
            raise TypeError(
                "Can not subclass a DataPipe[{}] from DataPipe[{}]".format(t, self.type)
            )
...
def _dp_init_subclass(sub_cls, *args, **kwargs):
    ...
    if isinstance(sub_cls.type.param, ForwardRef):
        ...
        try:
            ...
            raise TypeError(
                "{} is not supported by Python typing".format(
                    sub_cls.type.param.__forward_arg__
                )
            ) from e
        ...
        if "return" in hints:
            ...
            if not issubtype(data_type, sub_cls.type.param):
                raise TypeError(
                    ...
                    " for {}".format(
                        sub_cls.type, _type_repr(data_type), sub_cls.__name__
                    )
                )
...
def reinforce_type(self, expected_type):
    ...
    if not issubtype(expected_type, self.type.param):
        raise TypeError(
            "Expected 'expected_type' as subtype of {}, but found {}".format(
                self.type, _type_repr(expected_type)
            )
        )
