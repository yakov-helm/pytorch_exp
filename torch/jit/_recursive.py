...
def _get_valid_constant(attr, v, owner_type):
    ...
    raise TypeError(
        ...
        """.format(
                torch.typename(type(v)), owner_type, attr, constants
            )
        )
    )
...
def create_script_module_impl(nn_module, concrete_type, stubs_fn):
    ...
    def init_fn(script_module):
        ...
        for name, sub_concrete_type in concrete_type.get_modules():
            orig_value = getattr(nn_module, name)
            assert isinstance(orig_value, Module), "Expected Module but got {}".format(
                type(orig_value)
            )
...
def _check_no_signature(func):
    ...
    if signature is None:
        ...
        raise RuntimeError(
            "Must explicitly add type annotations to overloaded functions: {}".format(
                qual_name
            )
        )
...
def check_module_initialized(mod):
    ...
    if not hasattr(mod, "_parameters"):
        raise RuntimeError(
            "'{}' has not been initialized, did you forget to call 'super()'?".format(
                torch.typename(type(mod))
            )
        )
