...
def check_decomposition_has_type_annotations(f):
    ...
    for param in sig.parameters.values():
        ...
        ), "No signature on param {name} for function {func}".format(
            name=param.name, func=f.name
        )

    assert (
        sig.return_annotation != inspect_empty
    ), "No return annotation for function {func}".format(func=f.name)
...
def register_decomposition(aten_op, registry=None):
    def decomposition_decorator(f):
        ...
        ), "Duplicated function name {}".format(f.__name__)
