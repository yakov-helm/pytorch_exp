        ...
        for b in seg["blocks"]:
            ...
            if "history" in b:
                ...
                for h in b["history"]:
                    if active:
                        all_ranges.append(
                            (h["addr"] - seg["address"], h["real_size"], active)
                        )
        ...
        for i, t in enumerate(data["device_traces"]):
            if len(t) > 0:
                if device is not None:
                    raise ValueError(
                        f"Both device {device} and {i} have traces, use --device to specify which trace."
                    )
                device = i
    return device
...
def trace_plot(data, device=None, plot_segments=False):
    ...
    def add_element(addr, size, frames, extra=()):
        ...
        frames = [
            f"{addr_prefix}{addr:x}_{next_version - 1} {_format_size(size)} allocation ({size} bytes)",
            *extra,
        ...
        for b in seg["blocks"]:
            if b["state"] == "active_allocated" and addr not in addr_to_alloc:
                ...
                extra = (
                    ()
                    if frames
                    else ("<block was allocated before _record_history was enabled>",)
                )
...
def profile_plot(profile, device=None):
    ...
    for time, action, (tensor_key, version), size in memory_profile.timeline:
        ...
        if action == Action.CREATE:
            kv_to_elem[(tensor_key, version)] = elemid = add_element(
                size, tensor_key, version
            )
            w.allocate(elemid)
            ...
            w.free(kv_to_elem.pop((tensor_key, version)))
            kv_to_elem[(tensor_key, version + 1)] = elemid = add_element(
    ...
    def fold_free(ts):
        ...
        while i < len(ts):
            ...
            if i + 1 < len(ts):
                tnext = ts[i + 1]
                if (
                    t["action"] == "free_requested"
                    and tnext["action"] == "free_completed"
                    and t["addr"] == tnext["addr"]
                ):
                    yield {**t, "action": "free"}
                    i += 2
                    continue
    ...
    for seg in data["segments"]:
        ...
        for b in seg["blocks"]:
            if b["state"] in ("active_pending_free", "active_allocated"):
                ...
                blocks["pending_free"].append(
                    1 if b["state"] == "active_pending_free" else 0
                )
            addr += b["size"]
