...
def _is_conversion_supported(activation_post_process: torch.nn.Module) -> bool:
    ...
    return (
        (dtype in [torch.quint8, torch.qint8, torch.qint32] and (not is_dynamic))
...
def _insert_dequantize_node(node: Node, graph: Graph):
...
def convert(
    ...
    if qconfig_mapping:
        ...
        for k, v in node_name_to_qconfig.items():
            ...
            ), "Expected key {} in convert node_name_to_qconfig".format(k)
            if convert_node_name_to_qconfig[k] is not None:
                assert qconfig_equals(v, convert_node_name_to_qconfig[k]), (
                    ...
                    "but {} was updated to {}".format(
                        k, v, convert_node_name_to_qconfig[k]
                    )
                )
