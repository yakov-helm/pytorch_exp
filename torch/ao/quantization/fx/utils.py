    ...
    def _activation_post_process_satisfies_dtype_config_constraints(
        ...
        if backend_quant_min is not None and backend_quant_max is not None:
            if app_quant_min is None or app_quant_max is None:
                warnings.warn(
                    "QConfig %s must specify 'quant_min' and 'quant_max', ignoring %s"
                ...
                warnings.warn(
                    (
                        "QConfig %s quantization range must fall within the backend's:\n"
        ...
        if backend_scale_min is not None:
            if app_scale_min is None:
                warnings.warn(
                    "QConfig %s must specify 'eps', ignoring %s"
                    % (debug_string, qconfig)
                )
                return False
                ...
                warnings.warn(
                    (
                        "QConfig %s eps (%s) must be greater than or equal to "
        ...
        if (
            ...
            if not isinstance(
                ...
                warnings.warn(
                    (
                        "QConfig must specify a FixedQParamsObserver or a FixedQParamsFakeQuantize "
                        "for fixed qparams ops, ignoring %s.\n%s"
                    )
                    % (qconfig, suggestion_str)
                )
                return False
            if (
                ...
                warnings.warn(
                    (
                        "QConfig fixed scale (%s) and zero point (%s) do not match the backend's "
                        "(%s and %s), ignoring %s.\n%s"
                    )
                    % (
                        observer.scale,
                        observer.zero_point,
                        backend_scale_exact_match,
                        backend_zero_point_exact_match,
                        qconfig,
                        suggestion_str,
                    )
                )
