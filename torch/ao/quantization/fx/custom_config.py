...
class PrepareCustomConfig:
    ...
    def from_dict(
        ...
        def _get_qconfig_mapping(obj: Any, dict_key: str) -> Optional[QConfigMapping]:
            ...
            raise ValueError(
                "Expected QConfigMapping in prepare_custom_config_dict[\"%s\"], got '%s'"
                % (dict_key, type(obj))
            )
        ...
        def _get_prepare_custom_config(
            ...
            raise ValueError(
                "Expected PrepareCustomConfig in prepare_custom_config_dict[\"%s\"], got '%s'"
                % (dict_key, type(obj))
            )
        ...
        def _get_backend_config(obj: Any, dict_key: str) -> Optional[BackendConfig]:
            ...
            raise ValueError(
                "Expected BackendConfig in prepare_custom_config_dict[\"%s\"], got '%s'"
                % (dict_key, type(obj))
            )
