...
from ..observer import ObserverBase, _is_activation_post_process
from ..qconfig import (
...
def _get_dtype_and_is_dynamic(
    obs_or_fq_ctr: Optional[Callable],
...
def _insert_observer(
    ...
    with graph.inserting_after(node):
        new_obs = graph.create_node("call_module", observer_name, (node,), {})
    return new_obs
...
def _get_arg_target_dtype_as_output(
    ...
    if custom_module_lstm_node is not None:
        ...
        output_act_obs_or_fq_ctr = arg.meta["target_dtype_info"][
            "output_act_obs_or_fq_ctr"
        ]
...
def _maybe_insert_input_observer_for_arg_or_kwarg(
    ...
    if not is_standalone_module:
        ...
        act_post_process_ctr = (
            node.meta["target_dtype_info"]["weight_obs_or_fq_ctr"]
            if is_weight
            else node.meta["target_dtype_info"]["input_act_obs_or_fq_ctr"]
        )
        ...
        arg_as_input_target_dtype = _get_arg_target_dtype_as_input_to_node(
            arg, node, named_modules
        )
        arg_as_input_target_is_dynamic = _get_arg_target_is_dynamic_as_input_to_node(
            arg, node, named_modules
        )  # type: ignore[arg-type]
        needs_obs = (
            # the following code block is for static quantization
            (not arg_as_input_target_is_dynamic)
            and
            ...
            (arg_as_output_target_dtype not in _DO_NOT_OBS_DTYPE_LIST)
            and
        ...
        if cur_input_idx is None:
            ...
            needs_obs = (arg_as_output_target_dtype != arg_as_input_target_dtype) and (
                arg_as_input_target_dtype != torch.float
            )
    ...
    if needs_obs:
        ...
        for maybe_obs_node, _ in arg.users.items():
            if maybe_obs_node.op == "call_module":
                ...
                if (
                    type(maybe_obs_mod) == type(new_obs_mod)
...
def _maybe_insert_output_observer_for_node(
    ...
    should_insert_observer = (
        should_insert_observer
        and target_dtype in [torch.quint8, torch.qint8, torch.qint32, torch.float16]
        and not is_dynamic
    )
    ...
    should_insert_observer = should_insert_observer and (not is_standalone_module)
...
def insert_observers_for_model(
    ...
    for (
        ...
        output_act_dtype, _ = _get_dtype_and_is_dynamic(
            node.meta["target_dtype_info"]["output_act_obs_or_fq_ctr"]
        )
                        ...
                        if _is_custom_module_lstm(
                            ...
                            if maybe_output_obs_node is not None:
                                ...
                                _is_observer_in_same_graph_ = (
                                    _is_observer_in_same_graph(node, named_modules)
                                )
