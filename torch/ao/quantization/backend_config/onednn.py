...
def _fuse_linear_bn_leaky_relu(is_qat, linear, bn, leaky_relu):
    ...
    if is_qat:
        raise NotImplementedError(
            "Cannot fuse train modules: {}".format((linear, bn, leaky_relu))
        )
    else:
        ...
        if fused_module is not None:
            ...
            raise NotImplementedError(
                "Cannot fuse eval modules: {}".format((linear, bn, leaky_relu))
            )
...
def _fuse_conv_bn_add_left(is_qat, add, bn_conv, _):
    ...
    if is_qat:
        raise NotImplementedError(
            "Cannot fuse train modules: {}".format((conv, bn, add))
        )
    else:
...
def _fuse_conv_bn_add_relu_left(is_qat, relu, add_pattern):
    ...
    if is_qat:
        raise NotImplementedError(
            "Cannot fuse train modules: {}".format((conv, bn, add, relu))
        )
    else:
        fused_conv = nn.utils.fusion.fuse_conv_bn_eval(conv, bn)
